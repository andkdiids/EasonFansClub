...
test('后台专辑和歌曲创建 API 均受 music_manage 权限保护', () => {
  ...
  const albumCreateData = album.slice(album.indexOf('data: {'), album.indexOf('include: { MusicSong: true }'))
  assert.doesNotMatch(albumCreateData, /coverUrl:/)
  ...
  assert.match(song, /coverUrl: toPublicMediaUrl\(optionalMusicText\(body\?\.coverUrl, 1000\) \|\| album\.coverUrl\)/)
})
...
test('封面 API 使用固定 WebP 路径和腾讯云 COS', () => {
  ...
  assert.match(route, /music-cover\/\$\{folder\}\/\$\{entityId\}\/source\.webp/)
  assert.match(route, /uploadImageVariantFamily/)
  assert.match(route, /original/)
  assert.match(route, /uploadMusicMedia\(\{ kind: 'cover'/)
  assert.match(route, /contentType as 'image\/jpeg' \| 'image\/png' \| 'image\/webp'/)
  assert.match(route, /variants: \['thumb-sm', 'thumb-md', 'card', 'large'\]/)
...
test('专辑墙、可展开全量专辑与轻量背景动效统一为音乐档案馆视觉', () => {
  ...
  assert.match(albums, /grid-cols-3[\s\S]*sm:grid-cols-3[\s\S]*md:grid-cols-4[\s\S]*xl:grid-cols-5/)
  assert.match(albums, /theme="dark"/)
  assert.match(home, /MusicAlbumArchiveShowcase/)
  assert.match(showcase, /grid-cols-3[\s\S]*sm:grid-cols-3[\s\S]*md:grid-cols-5[\s\S]*xl:grid-cols-6/)
...
test('音乐封面保持懒加载且轮播卡片使用隔离的原生图片层', () => {
  ...
  assert.match(cover, /loading=\{priority \? undefined : 'lazy'\}/)
