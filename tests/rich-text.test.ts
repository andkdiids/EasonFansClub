...
test('post APIs validate rich content and continue accepting the legacy plain content field', () => {
  ...
  assert.match(createSource, /validateRichPostContent\(body\.richContent\)/)
  assert.match(createSource, /extractPlainText\(richContentResult\.value\)/)
  assert.match(createSource, /richContent: normalizedRichContent as Prisma\.InputJsonValue/)
  assert.match(createSource, /content: input\.content/)
  assert.match(editSource, /validateRichPostContent\(body\.richContent\)/)
  assert.match(editSource, /extractPlainText\(richContentResult\.value\)/)
  assert.match(editSource, /richContent: nextRichContent === null \? Prisma\.DbNull/)
  assert.match(editSource, /content: rawContent/)
  assert.match(detailSource, /<RichPostContent/)
  assert.match(detailSource, /richContent=\{post\.moderationStatus === 'VIOLATION' \? null : post\.richContent\}/)
})
