...
test('editor, renderer and post APIs use structured content instead of raw HTML', () => {
  ...
  assert.match(editorSource, /toggleBulletList\(\)/)
  assert.match(editorSource, /toggleOrderedList\(\)/)
  assert.match(editorSource, /applyList\('bulletList'\)/)
  assert.match(editorSource, /applyList\('orderedList'\)/)
  assert.match(editorSource, /name: 'blockquote'/)
  ...
  assert.match(detailSource, /richContent=\{renderedRichContent\}/)
