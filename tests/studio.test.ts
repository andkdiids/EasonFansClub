...
test('制作模式、导出、移动端和错误提示均由工作台接线', () => {
  ...
  assert.match(editor, /画板大小/)
  assert.match(editor, /宽度/)
  assert.match(editor, /高度/)
  assert.match(editor, /应用尺寸/)
  assert.match(editor, /当前：\{settings\.width\} × \{settings\.height\} 格/)
  assert.doesNotMatch(editor, /customBoardSizeOpen/)
  assert.match(editor, /resizeConfirmation/)
  assert.match(editor, /缩小画板可能裁掉现有内容/)
  ...
  assert.match(css, /\.beadsSettingsPanel > \.panelSection:nth-child\(2\) > \.fieldGroup:nth-child\(6\)/)
  assert.doesNotMatch(css, /\.beadsSettingsPanel > \.panelSection:nth-child\(2\) > \.fieldGroup:nth-child\(5\)/)
...
test('拼豆图片替换与 Canvas pointer 交互保持在 viewport 内', () => {
  const editor = read('components/studio/StudioBeadsTool.tsx')
  const css = read('components/studio/studio.module.css')
  assert.match(editor, /sourceReplaceButton/)
  assert.match(editor, /sourceReplaceInput/)
  assert.match(editor, /canvasViewportRef/)
  assert.match(editor, /clampPanToViewport/)
  assert.match(editor, /event\.currentTarget\.setPointerCapture\(event\.pointerId\)/)
  assert.match(editor, /event\.currentTarget\.releasePointerCapture\(event\.pointerId\)/)
  assert.match(editor, /onPointerCancel=\{onCanvasPointerUp\}/)
  assert.match(editor, /editorTool === 'pan'/)
  assert.match(editor, /setIsPanning\(true\)/)
  assert.match(editor, /setIsPanning\(false\)/)
  assert.match(editor, /event\.preventDefault\(\)/)
  assert.match(editor, /pointersRef\.current\.size >= 2/)
  assert.match(editor, /pinchRef\.current/)
  assert.doesNotMatch(editor, /sourceImageUrl \? '更换图片'/)
  assert.match(css, /\.sourcePreviewImage \{[^}]*object-fit: contain/)
  assert.match(css, /\.sourceReplaceButton \{[^}]*opacity: 0/)
  assert.match(css, /\.sourcePreview:hover \.sourceReplaceButton/)
  assert.match(css, /\.canvasViewport \{[^}]*position: relative[^}]*overflow: hidden[^}]*touch-action: none/)
  assert.match(css, /\.canvasStage \{[^}]*will-change: transform/)
  assert.match(css, /\.previewStage \{[^}]*overflow: visible[^}]*touch-action: none/)
  assert.match(css, /\.canvasViewportPan \{[^}]*cursor: grab/)
  assert.match(css, /\.canvasViewportPanning \{[^}]*cursor: grabbing/)
  assert.match(css, /@media \(max-width: 767px\)[\s\S]*\.sourceReplaceButton \{[^}]*opacity: 1/)
  assert.match(css, /@media \(max-width: 767px\)[\s\S]*\.sourceReplaceText \{ display: none; \}/)
})
...
test('P3 公开广场、互动、审核和统一分享能力保持在 Studio 层', () => {
  ...
  assert.match(publicProject, /createBrandedBeadPatternPdf/)
...
test('作品广场整卡入口与详情中文 PDF/JPG 导出保持统一品牌', () => {
  const gallery = read('components/studio/StudioGallery.tsx')
  const detail = read('components/studio/StudioPublicProject.tsx')
  const exportSource = read('lib/studio/beads/branded-export.ts')
  const css = read('components/studio/studio.module.css')
  assert.match(gallery, /galleryCardLink/)
  assert.match(gallery, /STUDIO_GALLERY_RETURN_STORAGE_KEY/)
  assert.match(gallery, /galleryActions/)
  assert.doesNotMatch(gallery, /projectActionPrimary/)
  assert.match(detail, /publicBackButton/)
  assert.match(detail, /router\.back\(\)/)
  assert.match(detail, /router\.push\('\/studio\/gallery'\)/)
  assert.match(detail, /publicDownloadMenu/)
  assert.match(detail, /createBrandedBeadPatternJpg/)
  assert.match(detail, /createBrandedBeadPatternPdf/)
  assert.doesNotMatch(detail, /public project \/ beads/)
  assert.match(exportSource, /drawBrandedQrToCanvas/)
  assert.match(exportSource, /shareCardQrPayload/)
  assert.match(exportSource, /贝多芬与我 · 拼豆图纸/)
  assert.match(exportSource, /私家E院/)
  assert.match(exportSource, /EasonFansClub/)
  assert.match(exportSource, /材料统计/)
  assert.match(exportSource, /application\/pdf/)
  assert.match(exportSource, /image\/jpeg/)
  assert.match(css, /\.publicHeader \{[^}]*grid-template-columns/)
  assert.match(css, /\.publicBackButton \{[^}]*min-height: 36px/)
  assert.match(css, /\.publicDownloadMenu \{[^}]*position: absolute/)
  assert.match(css, /@media \(max-width: 767px\)[\s\S]*\.publicHeader \{ grid-template-columns/)
  assert.match(css, /@media \(max-width: 767px\)[\s\S]*\.publicDownloadMenu \{ position: fixed/)
})
