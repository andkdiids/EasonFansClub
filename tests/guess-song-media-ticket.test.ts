...
test('media gateway wiring preserves the legacy route and protects the new origin', () => {
  ...
  assert.match(nginx, /proxy_cache_bypass \$http_range/)
  assert.match(nginx, /proxy_no_cache \$http_range/)
  assert.match(nginx, /proxy_pass https:\/\/ecfc\.fans;/)
  assert.match(nginx, /if \(\$request_method !~ \^\(\?:GET\|HEAD\)\$\)/)
  assert.match(nginx, /proxy_set_header X-ECFC-Media-Ticket \$guess_song_media_ticket/)
  assert.doesNotMatch(nginx, /\bslice\s+1m/)
  assert.doesNotMatch(nginx, /proxy_pass https:\/\/ecfc\.fans\/api\//)
