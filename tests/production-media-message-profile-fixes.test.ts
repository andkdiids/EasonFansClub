...
test('背景上传使用显式 label 触发文件选择且只支持三种格式', () => {
  const backgroundEditor = read('components/ProfileBackgroundEditor.tsx')
  assert.match(backgroundEditor, /type="file"/)
  assert.match(backgroundEditor, /accept="image\/jpeg,image\/png,image\/webp"/)
  assert.match(backgroundEditor, /className="sr-only"/)
  assert.doesNotMatch(backgroundEditor, /accept="image\/jpeg,image\/png,image\/webp,image\/gif"/)
})

test('背景上传展示处理上传阶段并捕获超时和网络错误', () => {
  const backgroundEditor = read('components/ProfileBackgroundEditor.tsx')
  assert.match(backgroundEditor, /setUploading\(true\)/)
  assert.match(backgroundEditor, /上传中…/)
  assert.match(backgroundEditor, /uploadError instanceof DOMException && uploadError\.name === 'AbortError'/)
  assert.match(backgroundEditor, /uploadError instanceof Error/)
  assert.match(backgroundEditor, /finally \{/)
})
