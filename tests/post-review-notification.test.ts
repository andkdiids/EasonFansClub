...
test('审核通过 / 拒绝都在同一事务内给作者创建通知', () => {
  ...
  const transactionReturn = reviewRoute.indexOf('return updated')
...
test('只有审核状态实际变化时才通知，重复审核不重复发送', () => {
  const guardIndex = reviewRoute.indexOf('if (current.moderationStatus !== status) {')
  const notificationCreate = reviewRoute.indexOf('tx.notification.create')
  assert.ok(guardIndex > 0 && notificationCreate > guardIndex)
  // 通知创建在状态变化守卫之内（守卫结束于 friendActivity 之前不允许跨出）
  const blockEnd = reviewRoute.indexOf("if (status === 'APPROVED' && current.moderationStatus !== 'APPROVED')")
...
test('审核通过与拒绝的通知文案和链接正确', () => {
  ...
  assert.match(reviewRoute, /rejectionReason\s*\?\s*`你发布的帖子《\$\{current\.title\}》未通过审核。原因：\$\{rejectionReason\}`\s*:\s*`你发布的帖子《\$\{current\.title\}》未通过审核，请修改后重新提交。`/)
  // 链接指向用户可见的帖子详情页，而非后台
