...
test('Sharp 与 COS 上传具有生产约束和超时保护', () => {
  ...
  assert.match(coverProcessor, /failOn: 'none'/)
  assert.match(coverProcessor, /\.flatten\(\{ background: '#ffffff' \}\)/)
  assert.match(coverProcessor, /\.webp\(\{ quality: MUSIC_COVER_QUALITY \}\)/)
  assert.match(coverRoute, /error\.message\.trim\(\)\.slice\(0, 500\)/)
  assert.match(coverRoute, /`图片转换失败：\$\{detail\}`/)
