...
test('提交公开审核后只允许 PENDING 转换，审核成功后才写对应结果通知', () => {
  ...
  assert.match(reviewRoute, /where: \{ id: project\.id, reviewStatus: current\.reviewStatus \}/)
  assert.match(reviewRoute, /SELECT \\`id\\` FROM \\`StudioProject\\` WHERE \\`id\\` = \$\{project\.id\} FOR UPDATE/)
  ...
  const updateStart = reviewRoute.indexOf('updated = await prisma.$transaction')
...
test('审核中心展示已有缩略图，缺失时使用 Pattern Grid renderer，并提供完整详情', () => {
  ...
  assert.match(adminPanel, /审核请前往审核中心/)
