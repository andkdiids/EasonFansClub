...
test('专家模式使用手动输入并按规范化歌名比较', () => {
  ...
  assert.equal(normalizeGuessSongAnswer('富士山下（Live 2007）'), normalizeGuessSongAnswer('富士山下'))
  assert.equal(normalizeGuessSongAnswer('十面埋伏 - Studio Version'), normalizeGuessSongAnswer('十面埋伏'))
  assert.equal(normalizeGuessSongAnswer('浮夸 [Remastered 2005]'), normalizeGuessSongAnswer('浮夸'))
  assert.equal(normalizeGuessSongAnswer('十面埋伏 - 2004'), normalizeGuessSongAnswer('十面埋伏'))
  assert.notEqual(normalizeGuessSongAnswer('十面埋伏 2004年'), normalizeGuessSongAnswer('十面埋伏'))
  assert.notEqual(normalizeGuessSongAnswer('Liver'), normalizeGuessSongAnswer('Liv'))
  assert.notEqual(normalizeGuessSongAnswer('富士'), normalizeGuessSongAnswer('富士山下'))
  const session = source('lib/guess-song-session.ts')
