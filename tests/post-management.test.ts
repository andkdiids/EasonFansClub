...
test('所有设为精华入口先弹统一确认框，确认后才发起请求', () => {
  ...
  assert.doesNotMatch(reviewCenter, /nextIsFeatured|toggleFlag|PostFeatureConfirmDialog/)
...
test('加精与取消精华请求期间 loading + disabled，且服务端幂等逻辑保留', () => {
  ...
  // 审核中心不再重复挂载帖子加精/置顶管理；这些入口保留在详情与资料维护面。
  assert.doesNotMatch(reviewCenter, /flaggingId|reviewingId|取消精华|nextIsFeatured/)
  assert.match(detailMenu, /onCancel=\{\(\) => setFeatureConfirm\(null\)\}/)
  assert.match(listActions, /onCancel=\{\(\) => setFeatureConfirm\(null\)\}/)
  assert.doesNotMatch(reviewCenter, /PostFeatureConfirmDialog/)
...
test('管理端前端按操作者角色隐藏/禁用加精入口（服务端 403 仍必须存在）', () => {
  assert.doesNotMatch(reviewCenter, /isPrivilegedAuthorRole|仅超管可加精|currentUserRole/)
  assert.match(reviewRoute, /hasAdminPermission\(guard\.user, definition\.permission\)/)
...
test('审核列表 GET 支持 keyword（trim / 空恢复 / 中文可用 / 组合 status+page）', () => {
  assert.match(reviewRoute, /sanitizeText\(params\.get\('keyword'\), 80\)\.trim\(\)/)
  ...
  assert.match(reviewRoute, /id: keyword/)
  assert.match(reviewRoute, /User: \{ uid \}/)
...
test('审核页不暴露批量删除，也不保留仅服务于它的多选框和确认弹窗', () => {
  assert.doesNotMatch(reviewCenter, /批量删除/)
  assert.doesNotMatch(reviewCenter, /bulk-delete/)
  assert.doesNotMatch(reviewCenter, /bulkDeleting|bulkConfirmOpen|confirmBulkDelete/)
  assert.doesNotMatch(reviewCenter, /selectedIds|selectedCount|toggleSelect|本页全选|已选择/)
  assert.doesNotMatch(reviewCenter, /type="checkbox"/)
  assert.doesNotMatch(reviewCenter, /<ConfirmDialog/)
})

test('审核页的单条审核与其他合法管理操作仍保留', () => {
  assert.match(reviewCenter, /onApprove=\{\(\) => void decide\(item, 'APPROVE'\)\}/)
  assert.match(reviewCenter, /onReject=\{\(\) => openReject\(item\)\}/)
...
test('统一审核中心保留帖子审核、分区调整与图片查看能力', () => {
  ...
  assert.match(reviewCenter, /postDetails/)
