...
test('clinic consultation completion refreshes live Aspirin progress from the committed server stream', () => {
  ...
  assert.match(aspirin, /prisma\.clinicConsultation\.findMany\([\s\S]*authorId: userId[\s\S]*record: \{ category: 'ASK_DOCTORS' \}/)
...
test('tracking refresh uses the persisted server state instead of an optimistic-only badge task value', () => {
  const component = read('components/BadgeCollectionPanel.tsx')
  const listRoute = read('app/api/users/me/badge-tasks/route.ts')
  const mutationRoute = read('app/api/users/me/badge-tasks/[badgeId]/route.ts')
  assert.match(component, /readBadgeTrackingState\(displayBadge\.id\)/)
  assert.match(component, /cache: 'no-store'/)
  assert.match(component, /setTracked\(state\.tracked\)/)
  assert.match(listRoute, /getBadgeTaskCenter\(guard\.user\.id\)/)
  assert.match(mutationRoute, /trackBadge\(guard\.user\.id, badgeId\)/)
  assert.match(mutationRoute, /untrackBadge\(guard\.user\.id, badgeId\)/)
})
