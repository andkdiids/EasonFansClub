...
test('天使的礼物使用真实开奖与合成记录，并提供主题筛选和服务端汇总', () => {
  ...
  assert.match(center, /prisma\.pharmacyDraw\.count/u)
  assert.match(center, /_sum: \{ drawCost: true \}/u)
  assert.match(center, /prisma\.pharmacyRecycleLog\.count/u)
  assert.match(center, /campaignTitle/u)
  ...
  assert.match(api, /const themeId = sanitizeText\(params\.get\('themeId'\)/u)
  assert.match(api, /themeId,/u)
  assert.match(client, /ANGEL_GIFT: '天使的礼物'/u)
  assert.match(client, /aria-label="主题奖池"/u)
  assert.match(client, /总抽奖次数/u)
