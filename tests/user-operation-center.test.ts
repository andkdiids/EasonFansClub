...
test('天使的礼物使用真实开奖与合成记录，并提供主题筛选和服务端汇总', () => {
  const center = read('lib/user-operation-center.ts')
  const api = read('app/api/admin/user-operations/route.ts')
  const client = read('app/admin/user-operations/UserOperationCenter.tsx')

  assert.equal(normalizeUserOperationCategory('ANGEL_GIFT'), 'ANGEL_GIFT')
  assert.match(center, /ANGEL_GIFT: '天使的礼物'/u)
  assert.match(center, /prisma\.pharmacyDraw\.findMany/u)
  assert.match(center, /prisma\.pharmacyRecycleLog\.findMany/u)
  assert.match(center, /prisma\.pharmacyDraw\.groupBy/u)
  assert.match(center, /by: \['campaignId'\]/u)
  assert.match(center, /_sum: \{ drawCost: true \}/u)
  assert.match(center, /_min: \{ drawAt: true \}/u)
  assert.match(center, /_max: \{ drawAt: true \}/u)
  assert.match(center, /prisma\.pharmacyRecycleLog\.groupBy/u)
  assert.match(center, /prisma\.pharmacyCampaign\.findMany/u)
  assert.match(center, /aggregateAngelGiftCampaignStats/u)
  assert.match(center, /campaignName/u)
  assert.match(center, /isDuplicate/u)
  assert.match(center, /DUPLICATE_COMBINE/u)
  assert.match(center, /audit: \{ themeId: row\.campaignId, drawId: row\.id \}/u)
  assert.match(center, /audit: \{ themeId: row\.campaignId, combineId: row\.id \}/u)
  assert.match(center, /totalRecordCount/u)
  assert.match(center, /pharmacyDrawId: null, pharmacyRecycleLogId: null/u)
  assert.match(api, /const campaignId = sanitizeText\(params\.get\('campaignId'\) \|\| params\.get\('themeId'\)/u)
  assert.match(api, /campaignId,/u)
  assert.match(client, /ANGEL_GIFT: '天使的礼物'/u)
  assert.match(client, /aria-label="主题奖池"/u)
  assert.doesNotMatch(client, /总抽奖次数/u)
  assert.match(client, /参与主题/u)
  assert.match(client, /campaignOptions/u)
  assert.match(client, /暂无执药记录/u)
  assert.match(client, /重复勋章合成/u)
})
