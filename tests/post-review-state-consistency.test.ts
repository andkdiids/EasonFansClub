...
test('普通用户编辑已审核帖子会开启新的审核周期，管理员编辑保持原有豁免', () => {
  assert.match(postEditRoute, /!isAdmin\s*\?\s*\{[\s\S]*moderationStatus: 'PENDING'/)
  ...
  assert.match(postEditRoute, /message: isAdmin \? '帖子已保存' : '修改已保存，正在等待审核/)
  assert.match(postEditRoute, /checkPostForbiddenWords\(\{ title: rawTitle, content: rawContent \}, user\)/)
  assert.match(postEditRoute, /reviewNotificationKey = isAdmin \? null : `post-review:\$\{postId\}:\$\{randomUUID\(\)\}`/)
  ...
  assert.match(publicModulesRoute, /viewer\.id === target\.id \|\| isAdminRole\(viewer\.role\)/)
