...
test('clinic consultation input preserves business validation and safe diagnostics', () => {
  ...
  assert.match(service, /ASPIRIN_MIN_LENGTH/)
  assert.match(service, /getAspirinConsultationLength/)
  assert.match(service, /record\.category === 'ASK_DOCTORS'/)
  assert.match(service, /ASPIRIN_CONTENT_TOO_SHORT/)
  ...
  assert.match(detail, /getAspirinConsultationLength/)
  assert.match(detail, /ASPIRIN_MIN_LENGTH/)
