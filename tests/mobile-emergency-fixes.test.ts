...
test('深色主题的挂号说明、评论和楼中楼使用语义 surface', () => {
  ...
  assert.match(messages, /checkin-comment-thread mt-2 space-y-2/)
  assert.doesNotMatch(messages, /checkin-comment-thread[^\n]*rounded-2xl/)
  assert.doesNotMatch(messages, /checkin-comment-thread[^\n]*p-2/)
  assert.match(messages, /checkin-comment-card/)
  assert.doesNotMatch(messages, /checkin-reply-thread[^\n]*border-l-2/)
  assert.match(replies, /post-reply-card/)
  assert.doesNotMatch(replies, /post-reply-thread[^\n]*border-l-2/)
  ...
  assert.doesNotMatch(css, /\.checkin-messages-panel \.checkin-comment-thread[^\n]*background/)
  assert.match(css, /\.checkin-messages-panel \.checkin-reply-thread,[\s\S]*\.post-replies-section \.post-reply-thread \{ background:transparent; border:0; box-shadow:none; outline:none; \}/)
})
