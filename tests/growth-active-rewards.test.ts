...
test('正式站内分享成功后才请求一次性每日分享奖励，打开分享方式不直接奖励', () => {
  ...
  assert.match(endpoint, /function shareSourceKey\(dateKey: string\)/)
  assert.match(endpoint, /return `content:\$\{dateKey\}`/)
  assert.match(endpoint, /sourceEventId: shareSourceKey\(dateKey\)/)
