...
test('clipboard image paths intercept only real image files and share the upload helper', () => {
  assert.match(editorSource, /clipboardImageFilesFromPaste\(event\)/u)
  assert.match(browserUploadSource, /const items = event\.clipboardData\?\.items/u)
  assert.match(browserUploadSource, /item\.kind !== 'file' \|\| !itemType\.startsWith\('image\/'\)/u)
  assert.match(browserUploadSource, /item\.getAsFile\(\)/u)
  assert.match(editorSource, /event\.preventDefault\(\)/u)
  assert.match(editorSource, /return false/u)
  assert.match(browserUploadSource, /navigator\.clipboard/u)
  assert.match(browserUploadSource, /clipboard\.read\(\)/u)
  assert.match(browserUploadSource, /item\.types\.find/u)
  assert.match(browserUploadSource, /item\.getType\(type\)/u)
  assert.match(browserUploadSource, /new File\(/u)
