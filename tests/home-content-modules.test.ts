...
test('homepage entertainment and daily music cards share a compact two-column row', () => {
  const surface = read('components/HomeLayoutSurface.tsx')
  const css = read('app/globals.css')
  ...
  const secondaryStart = css.indexOf('.home-secondary-columns {')
  const tabletStart = css.indexOf('@media (min-width: 768px) and (max-width: 1199px)', secondaryStart)
  assert.ok(secondaryStart >= 0)
  assert.ok(tabletStart > secondaryStart)
  const secondary = css.slice(secondaryStart, tabletStart)
  assert.match(secondary, /\.home-secondary-columns \{[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);/)
  assert.match(secondary, /\.home-secondary-columns > \.community-panel \{[\s\S]*min-height: 220px;[\s\S]*height: 100%;/)
  assert.match(secondary, /\.home-secondary-columns > \.home-activities-section \{ grid-column: 1 \/ -1; \}/)
  assert.match(secondary, /\.home-secondary-columns \.home-daily-music \{[\s\S]*grid-template-columns: 64px minmax\(0, 1fr\) auto;/)
  assert.match(secondary, /\.home-secondary-columns \.home-daily-music-lyrics \{[\s\S]*-webkit-line-clamp: 2;[\s\S]*line-clamp: 2;/)
  assert.match(secondary, /\.home-secondary-columns \.home-entertainment-mobile-score \{[\s\S]*display: block;/)
  assert.match(secondary, /\.home-secondary-columns \.home-entertainment-desktop-scores \{[\s\S]*display: none;/)

  const mobileStart = css.indexOf('@media (max-width: 767px)', tabletStart)
  const narrowStart = css.indexOf('@media (max-width: 339px)', mobileStart)
  assert.ok(mobileStart > tabletStart)
  assert.ok(narrowStart > mobileStart)
  const mobile = css.slice(mobileStart, narrowStart)
  assert.match(mobile, /\.home-secondary-columns \{[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[\s\S]*gap: 10px;/)
  assert.match(mobile, /\.home-secondary-columns > \.community-panel \{[\s\S]*min-height: 220px;[\s\S]*height: 100%;/)
  assert.match(mobile, /\.home-secondary-columns \.home-daily-music \{[\s\S]*grid-template-columns: 44px minmax\(0, 1fr\) auto;/)
  assert.match(css.slice(narrowStart), /\.home-secondary-columns \{[\s\S]*grid-template-columns: minmax\(0, 1fr\);/)
  assert.match(css.slice(narrowStart), /\.home-secondary-columns > \.home-activities-section \{ grid-column: auto; \}/)
})
