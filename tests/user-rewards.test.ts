...
test('奖励事务复用经验值与挂号费流水，绕过每日经验上限并防止重复请求', () => {
  const constants = read('lib/user-reward-constants.ts')
  const service = read('lib/user-rewards.ts')
  const legacyNotificationTypeConstant = ['USER', 'REWARD', 'NOTIFICATION', 'TYPE'].join('_')
  assert.match(constants, /ACTIVITY_NOTIFICATION_TYPE\s*=\s*'ACTIVITY'/)
  assert.doesNotMatch(constants, new RegExp(legacyNotificationTypeConstant))
  ...
  assert.match(service, /type: ACTIVITY_NOTIFICATION_TYPE/)
  assert.doesNotMatch(service, new RegExp(`type:\s*${legacyNotificationTypeConstant}`))
  assert.match(service, /createNotification\(notification\)/)
  assert.doesNotMatch(service, /tx\.notification\.create/)
...
test('用户奖励入口和 API 使用独立权限，且通知接入统一通知中心', () => {
  ...
  assert.match(notifications, /ACTIVITY:\s*'活动'/)
  assert.match(client, /user-reward:/)
