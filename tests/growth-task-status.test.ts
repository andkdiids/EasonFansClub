...
test('签到和处方事实在任务实例缺失时仍恢复今天状态，并统一为 2/6', () => {
...
test('点赞和分享以整项达标计数，不把每次事件拆成多个今日任务', () => {
  ...
  assert.equal(fourLikes.total, 6)
...
test('前端只消费服务端的今日 total/completed，并将六项渲染为一个列表', () => {
  const panel = read('components/GrowthPanel.tsx')
  assert.match(panel, /today: \{ dateKey: string; total: number; completed: number;/)
