...
test('服务端 Match snapshot 在一个 transaction 内读取题目和本题答案，并携带 revision', () => {
  ...
  assert.match(service, /export async function getDuelMatchState[\s\S]*?return prisma\.\$transaction\(async \(tx\) => \{[\s\S]*?loadQuestionState\(tx, matchId, match\.currentQuestionIndex\)[\s\S]*?guessSongDuelAnswer\.findMany/)
