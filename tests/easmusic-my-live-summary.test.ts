...
test('不同标题和简介长度使用统一居中 Modal 高度与固定内容槽位', () => {
  ...
  assert.match(css, /\.music-concert-gallery-modal-scroll \{[^}]*height:100dvh/)
  ...
  assert.match(css, /\.music-concert-gallery-modal-card \{[^}]*overflow:hidden/)
  assert.match(css, /@media \(max-width:767px\)[\s\S]*\.music-concert-gallery-modal-card \{[^}]*overflow-x:hidden[^}]*overflow-y:auto/)
