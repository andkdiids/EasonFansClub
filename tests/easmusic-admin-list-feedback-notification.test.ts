...
test('unread-summary polling is visibility-aware and runs every 30 seconds', () => {
  ...
  assert.match(provider, /const POLL_INTERVAL_MS = 30_000/)
  assert.match(provider, /fetch\('\/api\/notifications\/unread-summary'/)
  assert.match(provider, /window\.setInterval\(\(\) => void refresh\(\), POLL_INTERVAL_MS\)/)
  ...
  assert.match(provider, /console\.debug\('\[notification poll\]', Date\.now\(\)\)/)
