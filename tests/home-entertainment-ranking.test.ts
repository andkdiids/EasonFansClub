...
test('首页娱乐天空排行玩家列只显示固定头像和昵称，并为长昵称保留分数列', () => {
  ...
  assert.match(player, /home-entertainment-score-avatar/)
  assert.match(player, /home-entertainment-score-name">\{name\}<\/Link>/)
  assert.doesNotMatch(player, /UserDisplayName|equippedBadge|Badge|badge/)
  ...
  assert.match(styles, /\.home-secondary-columns \.home-entertainment-mode-score \{[\s\S]*grid-template-columns: minmax\(0, 1\.05fr\) minmax\(0, 1\.2fr\) max-content;/)
  assert.match(styles, /\.home-secondary-columns \.home-entertainment-player-cell \{[\s\S]*display: flex;[\s\S]*width: 100%;[\s\S]*min-width: 0;/)
  assert.match(styles, /\.home-secondary-columns \.home-entertainment-score-user \{[\s\S]*gap: 6px;/)
  assert.match(styles, /\.home-secondary-columns \.home-entertainment-score-avatar \{[\s\S]*width: 20px;[\s\S]*height: 20px;[\s\S]*flex: 0 0 auto;/)
  assert.match(styles, /\.home-secondary-columns \.home-entertainment-score-name \{[\s\S]*min-width: 0;[\s\S]*flex: 1 1 0;/)
  ...
  assert.match(styles, /@media \(max-width:767px\) \{[\s\S]*grid-template-columns: minmax\(0, 1fr\) minmax\(0, 1fr\) max-content;/)
})
