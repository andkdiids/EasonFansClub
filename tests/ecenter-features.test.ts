...
test('没有数据库覆盖时使用 Registry 默认顺序，勋章展览馆不强制排第一', () => {
  ...
  const studioIndex = features.findIndex((feature) => feature.featureKey === 'STUDIO')
  assert.equal(studioIndex, 11)
  assert.equal(features[studioIndex]?.label, '贝多芬与我')
  assert.equal(features[studioIndex]?.href, '/studio')
  assert.equal(features.findIndex((feature) => feature.featureKey === 'BADGE_MUSEUM'), 17)
