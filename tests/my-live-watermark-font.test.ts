...
test('水印 SVG 使用解析后的中文字体，且包含中文用户名原文（UTF-8 字面量）', () => {
  const { svg, text } = buildMyLivePhotoWatermarkSvg({ username: '范范Vanessa', uid: 11153, width: 1200, height: 1600 })
...
test('用户名中的 XML 特殊字符被正确转义', () => {
  const { text, svg } = buildMyLivePhotoWatermarkSvg({ username: 'A&B<C>', uid: 1, width: 800, height: 600 })
...
test('emoji 用户名渲染不崩溃（emoji 字体按支持情况回退）', async () => {
  ...
  const overlay = buildMyLivePhotoWatermarkSvg({ username: '范😀Vanessa', uid: 11153, width: 800, height: 600 })
...
test('实际渲染：含中文用户名的水印可生成有效 WebP（不崩溃）', async () => {
  ...
  const overlay = buildMyLivePhotoWatermarkSvg({ username: '范范Vanessa', uid: 11153, width: 1200, height: 1600 })
  const out = await sharp(base)
    .composite([{ input: Buffer.from(overlay.svg), left: overlay.left, top: overlay.top }])
    .webp()
    .toBuffer()
