...
test('E院广场只挂载小臣书流，详情页仍按移动端边界启用发现式布局', () => {
  ...
  assert.match(home, /<ForumDiscoveryHome previewMode=\{previewMode\} showDesktopRefresh=\{!previewMode\} \/>/)
...
test('小臣书首页只挂载一个 feed，请求具备取消、去重和错误停止自动重试保护', () => {
  ...
  assert.match(home, /ForumDiscoveryHome previewMode=\{previewMode\}/)
  assert.doesNotMatch(home, /useState|useEffect|matchMedia|localStorage/)
  assert.match(discovery, /previewMode \? previewPosts : \[\]/)
  ...
  assert.match(discovery, /\[hasMore, loadPage, loadingMore, previewMode\]/)
