...
test('system discovery tabs use server-side sorting and never expose post-card IP regions', () => {
  ...
  assert.match(route, /isPinned: 'desc'[\s\S]*likeCount: 'desc'[\s\S]*replyCount: 'desc'[\s\S]*createdAt: 'desc'[\s\S]*id: 'desc'/)
  ...
  assert.doesNotMatch(route, /isFeatured:\s*false/)
  assert.doesNotMatch(route, /isFeatured:\s*'desc'/)
