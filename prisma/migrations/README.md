...
- The release workflow runs `pnpm migration:check:mysql` but currently fails closed before production `migrate deploy`; production schema changes require a separately reviewed manual SQL procedure until migration history reconciliation is complete.
