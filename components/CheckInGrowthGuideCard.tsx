...
export function CheckInGrowthGuideCard({ compact = false }: Readonly<{ compact?: boolean }>) {
  return (
    <details className="checkin-growth-guide group overflow-hidden rounded-sm border shadow-sm">
      <summary className={`checkin-growth-guide-summary flex cursor-pointer list-none items-center justify-between gap-3 font-black text-brand-950 outline-none transition focus-visible:ring-2 focus-visible:ring-brand-300 [&::-webkit-details-marker]:hidden ${compact ? 'px-3 py-2 text-xs' : 'px-4 py-3 text-sm'}`}>
        <span className="flex min-w-0 items-center gap-2"><span aria-hidden="true">🏥</span><span>了解经验值与挂号费</span></span>
        <span aria-hidden="true" className="checkin-growth-guide-toggle grid h-6 w-6 shrink-0 place-items-center rounded-sm text-base transition-transform group-open:rotate-45">＋</span>
      </summary>

      <div className={`border-t ${compact ? 'p-3' : 'p-4 sm:p-5'}`}>
        <div className="checkin-growth-guide-intro border-l-4 border-brand-700 px-4 py-3">
        ...
        <div className="checkin-growth-guide-callout mt-4 border border-blue-200 px-4 py-3 text-center text-sm font-black text-brand-950">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <GuideSection eyebrow="每日奖励" title="如何获取挂号费">
            ...
            <p className="checkin-growth-guide-note border-l-2 border-amber-400 px-3 py-2 text-xs text-amber-800">中断后需重新连续挂号 7 天，奖励会按各自规则完整发放。</p>
          </GuideSection>
