...
export function SecurityQuestionsForm() {
  ...
  return (
    <form onSubmit={submit} className="space-y-5 rounded-[28px] border border-sky-100 bg-white/90 p-6 shadow-sm">
      <div className="rounded-2xl bg-amber-50 px-4 py-3 text-sm font-black leading-7 text-amber-800">
      ...
      {questions.map((item, index) => (
        <section key={index} className="space-y-3 rounded-2xl bg-sky-50/60 p-4">
          ...
          <input value={item.question} onChange={(event) => update(index, 'question', event.target.value)} required maxLength={120} className="w-full rounded-xl border border-sky-100 bg-white px-4 py-3 font-bold outline-none focus:border-brand-500" placeholder="请自行填写问题内容" />
          <input value={item.answer} onChange={(event) => update(index, 'answer', event.target.value)} required maxLength={200} autoComplete="off" className="w-full rounded-xl border border-sky-100 bg-white px-4 py-3 font-bold outline-none focus:border-brand-500" placeholder="填写答案" />
        </section>
      ))}
