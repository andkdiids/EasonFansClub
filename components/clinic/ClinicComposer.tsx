  ...
  return (
    <main className="clinic-page-shell clinic-new-page">
      ...
      <section className="clinic-form-panel">
        <section className="clinic-form-step"><h2 className="clinic-form-step-title">1. 今日症状</h2><div className="clinic-option-fieldset"><div className="clinic-option-grid">{clinicCategoryOptions.map((item) => <label key={item.value} className={category === item.value ? 'is-selected' : ''}><input type="radio" name="clinic-category" value={item.value} checked={category === item.value} onChange={() => setCategory(item.value)} /><span><b>{item.label}</b><small>{item.description}</small></span></label>)}</div></div></section>
        <section className="clinic-form-step"><h2 className="clinic-form-step-title">2. 我现在需要</h2><div className="clinic-option-fieldset"><div className="clinic-need-grid">{clinicNeedOptions.map((item) => <label key={item.value} className={needType === item.value ? 'is-selected' : ''}><input type="radio" name="clinic-need" value={item.value} checked={needType === item.value} onChange={() => setNeedType(item.value)} /><span>{item.label}</span></label>)}</div></div></section>
        <section className="clinic-form-step"><h2 className="clinic-form-step-title">3. 身份</h2><div className="clinic-option-fieldset"><div className="clinic-identity-options">{clinicIdentityOptions.map((item) => <label key={item.value} className={identityMode === item.value ? 'is-selected' : ''}><input type="radio" name="clinic-identity" value={item.value} checked={identityMode === item.value} onChange={() => setIdentityMode(item.value)} /><span><b>{item.label}</b>{item.value === 'ANONYMOUS' ? <small>普通病友无法看到你的真实身份。</small> : null}</span></label>)}</div></div></section>
