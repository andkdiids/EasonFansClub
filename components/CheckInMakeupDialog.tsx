...
type PaidResponse = { balance?: number; created?: boolean }
...
export function CheckInMakeupDialog({
  ...
  async function startChallenge() {
    ...
    setMessage('')
    try {
      const data = await fetchCheckInMakeupJson<ChallengeResponse>('/api/checkin/makeup/challenge', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ targetDate }),
      }, '挑战创建失败')
      if (!data?.challenge) throw new Error('挑战创建失败，请稍后重试')
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : '挑战创建失败') } finally { finishBusy() }
  }
  ...
  async function submitAnswer() {
    ...
    setMessage('')
    try {
      const data = await fetchCheckInMakeupJson<AnswerResponse>(`/api/checkin/makeup/challenge/${challenge.challengeId}/answer`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ selectedOptionId }),
      }, '答案提交失败')
      audioRef.current?.pause()
      const answerResult = { status: data.status, correctAnswer: data.correctAnswer || '', madeUp: Boolean(data.madeUp), settlementError: data.settlementError }
      setResult(answerResult)
      if (data.madeUp) {
        const successMessage = `补签成功，已补签 ${dateLabel}。`
        window.dispatchEvent(new Event('checkin:completed'))
        onCompleted(successMessage)
      }
    } catch (error) { setMessage(error instanceof Error ? error.message : '答案提交失败') } finally { finishBusy() }
  }
  ...
  async function pay() {
    ...
    setMessage('')
    try {
      const data = await fetchCheckInMakeupJson<PaidResponse>('/api/checkin/makeup/paid', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ targetDate }),
      }, '补签失败')
      ...
      window.dispatchEvent(new Event('checkin:completed'))
      onCompleted(successMessage)
    } catch (error) { setMessage(error instanceof Error ? error.message : '补签失败') } finally { finishBusy() }
  }
  ...
  const dialog = (
    <article className="checkin-history-detail" role="dialog" aria-modal="true" aria-labelledby="makeup-title">
        ...
        <div className="checkin-history-detail-body space-y-4">
          ...
          {result?.status === 'WRONG' ? <div className="space-y-2"><h3 className="text-xl font-black text-rose-700">回答错误</h3><p>正确答案：{result.correctAnswer}</p><p>本月免费补签挑战已使用。仍可使用 {cost} 挂号费完成本次补签。</p><button type="button" className="min-h-11 bg-brand-700 px-4 font-black text-white disabled:opacity-50" disabled={busy} onClick={() => setConfirmPaid(true)}>使用{cost}挂号费补签</button><button type="button" className="ml-2 min-h-11 border border-slate-300 px-4 font-black" onClick={onClose}>暂不补签</button></div> : null}
          {!result && challenge?.status === 'PENDING' ? <div className="space-y-4"><p className="font-black">免费补挂号挑战</p><p>听 {challenge.audio.durationSeconds} 秒前奏，猜猜这是哪首歌？</p><button type="button" className="min-h-11 bg-brand-950 px-4 font-black text-white" onClick={() => void playIntro()}>播放10秒前奏</button><div className="grid gap-2">{challenge.options.map((option) => <label key={option.id} className="flex min-h-11 items-center gap-3 border border-slate-200 px-3"><input type="radio" name="makeup-answer" value={option.id} checked={selectedOptionId === option.id} onChange={() => setSelectedOptionId(option.id)} /><span>{option.label}</span></label>)}</div><button type="button" className="min-h-11 bg-brand-700 px-4 font-black text-white disabled:opacity-50" disabled={busy || !selectedOptionId} onClick={() => void submitAnswer()}>{busy ? '提交中…' : '确认答案'}</button></div> : null}
          {!result && !challenge && !confirmPaid ? <div className="space-y-4"><p>补签计入连续挂号，但不会补发当天普通签到奖励。每周最多补签 1 次。</p>{monthlyChallengeAvailable || monthlyChallengePending ? <div className="border border-sky-200 bg-sky-50 p-3"><p className="font-black">本月还有一次免费补挂号机会</p><p>听10秒前奏，答对即可免费补签；答错仍可使用{cost}挂号费完成补签。每月仅一次。</p><button type="button" className="mt-3 min-h-11 bg-brand-700 px-4 font-black text-white" disabled={busy} onClick={() => void startChallenge()}>{busy ? '加载中…' : '开始免费挑战'}</button></div> : <p className="font-bold text-slate-600">本月免费挑战已使用。</p>}<button type="button" className="min-h-11 border border-brand-700 px-4 font-black text-brand-800 disabled:opacity-50" disabled={busy} onClick={() => setConfirmPaid(true)}>直接使用{cost}挂号费补签</button></div> : null}
          {confirmPaid && !result?.madeUp ? <div className="space-y-3 border border-amber-200 bg-amber-50 p-3"><p>将消耗 {cost} 挂号费。当前挂号费：{balance}</p><p>补签计入连续挂号，但不会补发当天原本的普通签到奖励。</p>{balance < cost ? <p className="font-black text-rose-700">挂号费不足：补签需要 {cost}，当前余额 {balance}。</p> : <button type="button" className="min-h-11 bg-brand-950 px-4 font-black text-white" disabled={busy} onClick={() => void pay()}>{busy ? '补签中…' : `确认补签 · ${cost}挂号费`}</button>}</div> : null}
  ...
  return embedded ? dialog : (
    <div className="checkin-history-detail-backdrop checkin-makeup-detail-host" onClick={(event) => { if (event.target === event.currentTarget) onClose() }}>
      {dialog}
    </div>
  )
}
