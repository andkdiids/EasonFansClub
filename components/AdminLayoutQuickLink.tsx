    ...
    <>
      ...
      {!inlineEdit ? (
        <div className="fixed bottom-24 right-4 z-40 flex flex-col gap-2 md:bottom-5">
          <Link href={inlineHref} className="rounded-full border border-sky-100 bg-brand-950 px-4 py-2 text-xs font-black text-white shadow-xl shadow-sky-900/20">
            前台编辑布局
          </Link>
          <Link href={`/admin/layout-editor?page=${pageKey}`} className="rounded-full border border-sky-100 bg-white px-4 py-2 text-xs font-black text-brand-700 shadow-xl shadow-sky-900/10">
            完整布局编辑器
          </Link>
        </div>
      ) : null}
