...
export function Topbar({ user, logoUrl, unreadCount, canManageLayout, canAccessAdmin }: Readonly<{ user: SessionUser; logoUrl: string | null; unreadCount: number; canManageLayout: boolean; canAccessAdmin: boolean }>) {
  ...
  return <header className={`app-topbar ${home ? 'app-topbar-home' : ''} ${inverse ? 'app-topbar-inverse' : ''}`}>
    ...
    <div className="app-topbar-actions">
      ...
      {menuOpen ? <div className="app-topbar-menu">
        <Link href="/profile">我的主页</Link>
        <Link href="/notifications">消息中心</Link>
        <Link href="/profile?module=favorites#profile-modules">我的收藏</Link>
        <Link href="/profile#checkin-records">签到记录</Link>
        <Link href="/settings/security">账号安全</Link>
        {canAccessAdmin ? <Link href="/admin" className="app-topbar-admin-link">后台管理</Link> : null}
