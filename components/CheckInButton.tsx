...
export function CheckInButton({
      ...
      <div className={isCompact ? 'space-y-2' : 'space-y-4'}>
        ...
        <div className={isCompact ? 'rounded-2xl border border-sky-100 bg-sky-50/70 p-3' : 'rounded-3xl border border-sky-100 bg-sky-50/70 p-5'}>
          ...
          {todayCheckIn.message ? (
            <p className={isCompact ? 'mt-2 line-clamp-2 whitespace-pre-wrap rounded-2xl bg-white/80 px-3 py-2 text-xs font-bold leading-5 text-slate-700' : 'mt-4 whitespace-pre-wrap rounded-2xl bg-white/80 px-4 py-3 text-sm font-bold leading-7 text-slate-700'}>{todayCheckIn.message}</p>
          ) : (
  ...
  return (
    <div className={isMinimal ? 'space-y-2' : isCompact ? 'space-y-3' : 'space-y-5'}>
      <div className="flex flex-wrap gap-2">
        <span className="rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-black text-brand-700 sm:text-xs">Lv.{stats.level}</span>
        <span className="rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-black text-brand-700 sm:text-xs">{stats.points} 积分</span>
        <span className="rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-black text-brand-700 sm:text-xs">{stats.exp} 经验</span>
      ...
      <div>
        ...
        <div className={isMinimal ? 'mt-1 grid grid-cols-5 gap-1' : isCompact ? 'mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-5' : 'mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5'}>
          {DAILY_MOODS.map((item) => (
            <button
              ...
              className={`${isMinimal ? 'min-h-8 rounded-lg p-1 text-center' : isCompact ? 'min-h-12 rounded-xl p-2' : 'min-h-20 rounded-2xl p-3'} border text-left transition ${
                mood === item.key
            ...
            >
              <span className={isMinimal ? 'text-lg leading-none' : isCompact ? 'text-xl' : 'text-3xl'}>{item.icon}</span>
              <span className={isMinimal ? 'sr-only' : isCompact ? 'ml-1 inline text-xs font-black text-slate-800 sm:ml-0 sm:block' : 'mt-2 block text-sm font-black text-slate-800'}>{item.label}</span>
            </button>
          ))}
        </div>
      </div>

      {!isMinimal ? <label className="block">
