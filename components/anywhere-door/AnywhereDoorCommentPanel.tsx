      ...
      <div className={`mt-4 space-y-3 ${embedded ? 'min-h-0 flex-1 lg:overflow-y-auto lg:pr-1' : ''}`}>
        {comments.length ? comments.map((comment) => (
          <div key={comment.id} id={`comment-${comment.id}`} className="border-b border-sky-100 pb-3 last:border-0 dark:border-slate-700">
            ...
            <p className="mt-2 whitespace-pre-wrap break-words text-sm leading-6 text-slate-700 dark:text-slate-300">{comment.content}</p>
            <div className="mt-2 flex gap-3 text-xs font-black text-brand-700 dark:text-sky-300"><button type="button" onClick={() => setReplyTo(comment)} className="hover:underline">回复</button>{comment.canDelete ? <button type="button" onClick={() => void remove(comment.id)} className="text-red-600 hover:underline dark:text-red-300">删除</button> : null}</div>
            {comment.replies.length ? <div className="mt-3 space-y-2 border-l-2 border-sky-200 pl-3 dark:border-slate-600">{comment.replies.map((reply) => <div key={reply.id} id={`comment-${reply.id}`} className="p-2"><div className="flex items-center justify-between gap-2 text-xs font-black text-brand-900 dark:text-slate-200"><span>{reply.author.nickname}</span><time dateTime={reply.createdAt}>{formatDate(reply.createdAt)}</time></div><p className="mt-1 whitespace-pre-wrap break-words text-sm leading-6 text-slate-700 dark:text-slate-300">{reply.content}</p>{reply.canDelete ? <button type="button" onClick={() => void remove(reply.id)} className="mt-1 text-xs font-black text-red-600 hover:underline dark:text-red-300">删除</button> : null}</div>)}</div> : null}
            {comment.replyCount > comment.replies.length || comment.repliesNextCursor ? <button type="button" onClick={() => void loadReplies(comment.id, comment.repliesNextCursor)} className="mt-2 text-xs font-black text-brand-700 hover:underline dark:text-sky-300">{comment.replies.length ? `查看更多回复（${comment.replyCount}）` : `查看 ${comment.replyCount} 条回复`}</button> : null}
          </div>
        )) : <p className="py-5 text-center text-sm font-bold text-slate-400">还没有评论，来留下第一句吧。</p>}
      </div>
      {nextCursor ? <button type="button" onClick={() => void loadComments(nextCursor)} className="mt-4 min-h-10 shrink-0 border border-sky-200 px-4 text-xs font-black text-brand-700 hover:bg-sky-50 dark:border-slate-600 dark:text-sky-300 dark:hover:bg-slate-800">加载更多评论</button> : null}
      <div className="mt-5 shrink-0 border-t border-sky-100 pt-4 dark:border-slate-700">
        ...
        <textarea value={content} onChange={(event) => setContent(event.target.value)} maxLength={500} rows={3} placeholder="写下你的评论…" className="w-full resize-y rounded-xl border border-sky-100 bg-white px-3 py-3 text-sm font-medium text-slate-800 outline-none ring-brand-200 focus:ring-2 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100" />
