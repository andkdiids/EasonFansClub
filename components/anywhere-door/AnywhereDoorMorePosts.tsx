...
export function AnywhereDoorMorePosts({ posts, currentPostId }: Readonly<{ posts: SocialPostView[]; currentPostId?: string }>) {
  ...
  return (
    <aside className="hidden min-w-0 lg:block" data-anywhere-door-more-posts>
      <div className="sticky top-6 border border-sky-100 bg-white/75 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900/75">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-sm font-black text-brand-950 dark:text-slate-100">更多帖子</h2>
        </div>
        {visiblePosts.length ? <div className="mt-4 grid grid-cols-2 gap-2">
          {visiblePosts.map((post) => {
            ...
            return <Link key={post.id} href={`/anywhere-door/${post.id}`} className="group block min-w-0" aria-label={`打开 @${post.authorUsername} 的动态`}>
              <div className="relative aspect-square overflow-hidden bg-black dark:bg-black">
                {media ? (
              ...
              <p className="mt-1.5 truncate text-xs font-bold text-slate-600 group-hover:text-brand-700 dark:text-slate-300 dark:group-hover:text-sky-300">{post.caption || `@${post.authorUsername}`}</p>
            </Link>
          })}
