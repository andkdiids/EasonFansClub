...
export function HomeLayoutSurface({ layoutConfig, siteConfig, slides, announcement }: { layoutConfig: PageLayoutConfig; siteConfig: SiteAppearanceConfig; slides: SiteHeroSlide[]; announcement: Announcement | null }) {
  ...
  return <div className="community-home">
      <section className="community-hero">
        ...
        <div className="community-hero-copy"><p>WELCOME HOME</p><h1>
  私家E院
</h1><h2>
  听见 Eason，也听见自己
</h2>
...
<em>C’mon in~</em><a href="#community-content" className="hero-primary-button">浏览今日内容 <span aria-hidden>›</span></a></div>
      </section>
  ...
  <div data-post-like-control className="pointer-events-auto relative z-[3]">
    ...
    />
...
</div></article>)}{!data.posts.length&&!failed?<p className="community-empty">{siteConfig.text.emptyText}</p>:null}</div></section>:null}
          <div className="community-right-column">
            ...
            {visible('home.culture')?<section className="community-panel concert-panel"><header><h2>{layoutModule('home.culture')?.title||'近期演唱会'}</h2><Link href="/activities">更多 ›</Link></header><div>{data.activities.map((activity)=><Link key={activity.id} href={`/activities/${activity.id}`}><time>{activity.startsAt?new Intl.DateTimeFormat('zh-CN',{month:'2-digit',day:'2-digit'}).format(new Date(activity.startsAt)):'待定'}</time><span><strong>{activity.title}</strong><small>{activity.description||'详情请见活动页面'}</small></span><b>{activityStatus(activity)}</b></Link>)}{!data.activities.length&&!failed?<p className="community-empty">暂无已发布活动。</p>:null}</div></section>:null}
          </div>
        </div>
      </div>
  </div>
