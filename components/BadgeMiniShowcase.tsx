...
export function BadgeMiniShowcase({ uid, summary, equippedBadges, equippedBadge, isSelf = false }: Props) {
  ...
  return <section className="badge-mini-showcase" aria-labelledby={`badge-mini-showcase-${uid}`}>
    ...
    {items.length ? <div className="badge-mini-showcase-cabinet">{items.map((item: BadgeMuseumItem, index) => <Link href={`${href}?badge=${encodeURIComponent(item.id)}`} key={item.id} className="badge-mini-showcase-item" aria-label={`${item.name}勋章详情`}><span className="badge-mini-showcase-image"><BadgeImage badge={item} size="wall" grayscale={isBadgeView(item) && item.isGrayed} /></span><BadgeName badge={item} className="badge-mini-showcase-name" />{isBadgeView(item) && item.isGrayed ? <span className="badge-mini-showcase-equipped">暂时失效</span> : isBadgeView(item) && item.isEquipped ? <span className="badge-mini-showcase-equipped">佩戴中</span> : index === 0 && equippedBadge?.id === item.id ? <span className="badge-mini-showcase-equipped">佩戴中</span> : null}</Link>)}</div> : <div className="badge-mini-showcase-empty"><span>还没有收藏勋章</span><Link href="/badges">去展览馆看看 →</Link></div>}
