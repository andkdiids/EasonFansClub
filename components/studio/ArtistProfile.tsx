...
import type { StudioArtistSummary, StudioGalleryProject } from '@/lib/studio/types'
...
function ArtistProjectCard({ project }: Readonly<{ project: StudioGalleryProject }>) {
...
export function ArtistProfile({ artist, projects }: Readonly<{ artist: StudioArtistSummary; projects: StudioGalleryProject[] }>) {
  return <main className={styles.artistPage}>
    <Link href="/studio/gallery" className={styles.artistBack}>← 返回创作广场</Link>
    <section className={styles.artistHero} aria-labelledby="artist-profile-title">
      <span className={styles.artistAvatar}>{artist.avatar ? <img src={artist.avatar} alt={`${artist.name}头像`} /> : artist.name.slice(0, 1)}</span>
      <div className="min-w-0">
        <span className={styles.artistKicker}>ARTIST</span>
        <h1 id="artist-profile-title" className={styles.artistName}>{artist.name}</h1>
        <p className={styles.artistDescription}>{artist.description || '用作品留下属于自己的声音。'}</p>
      </div>
    </section>
    <div className={styles.artistWorksHeader}><h2>作品列表</h2><span>{projects.length} 件公开作品</span></div>
    {projects.length ? <section className={styles.artistWorksGrid} aria-label={`${artist.name}作品列表`}>{projects.map((project) => <ArtistProjectCard key={project.id} project={project} />)}</section> : <div className={styles.artistEmpty}><p>暂时还没有公开作品</p><small>作品通过审核后，会出现在这里。</small></div>}
  </main>
}
