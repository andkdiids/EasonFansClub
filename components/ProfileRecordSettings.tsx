...
export function ProfileRecordSettings({ initialPreferences, onSaved }: Props) {
  ...
  return <div className="mb-3 min-w-0 border-b border-[var(--border)] pb-3">
    <button type="button" aria-expanded={open} onClick={() => setOpen((current) => !current)} className="inline-flex min-h-9 items-center rounded-sm border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs font-black text-[var(--primary)] transition hover:bg-[var(--surface-subtle)]">
      {open ? '收起记录管理' : '管理记录'}
    </button>
    {open ? <div className="mt-3 space-y-2" aria-label="个人记录管理">
      ...
      {preferences.map((preference, index) => {
        ...
        return <div key={preference.key} className="flex min-w-0 flex-wrap items-center gap-2 rounded-sm border border-[var(--border)] bg-[var(--surface)] px-2.5 py-2">
          ...
          <button type="button" role="switch" aria-label={`${label}${preference.visible ? '已显示' : '已隐藏'}`} aria-checked={preference.visible} disabled={saving} onClick={() => toggle(preference.key)} className={`rounded-sm border border-[var(--border)] px-2.5 py-1 text-[11px] font-black ${preference.visible ? 'bg-[var(--navigation-active)] text-[var(--primary)]' : 'bg-[var(--surface-subtle)] text-[var(--foreground-muted)]'} disabled:opacity-50`}>{preference.visible ? '显示' : '隐藏'}</button>
        </div>
      })}
      <div className="flex flex-wrap items-center gap-2 pt-1">
        <button type="button" disabled={saving} onClick={() => void save()} className="rounded-sm border border-[var(--primary)] bg-[var(--primary)] px-3 py-2 text-xs font-black text-[var(--primary-foreground)] disabled:cursor-wait disabled:opacity-60">{saving ? '保存中…' : '保存设置'}</button>
