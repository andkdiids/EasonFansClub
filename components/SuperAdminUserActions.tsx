...
type SuperAdminUserActionsProps = { targetUserId: string; initialUid: number; nickname: string }

function formatUid(value: number | string) {
  return String(Number(value)).padStart(5, '0')
}

export function SuperAdminUserActions({ targetUserId, initialUid, nickname }: Readonly<SuperAdminUserActionsProps>) {
  ...
  const [busy, setBusy] = useState<'uid' | 'password' | null>(null)
