...
export function MusicArchiveShell({ children, maxWidth = 'max-w-7xl', variant = 'archive', backgroundVisual }: Readonly<MusicArchiveShellProps>) {
  ...
  useEffect(() => {
    document.documentElement.classList.add('easmusic-route-active')
    document.body.classList.add('easmusic-route-active')
    return () => {
      document.documentElement.classList.remove('easmusic-route-active')
      document.body.classList.remove('easmusic-route-active')
    }
  }, [])
  ...
  return <main ref={shellRef} data-music-archive="true" className="relative isolate min-h-screen [overflow-x:clip] bg-[#06101d] text-white">
    ...
    <div className={`pointer-events-none fixed inset-0 z-0 ${isHome ? 'bg-[radial-gradient(circle_at_50%_35%,rgba(45,110,185,.20),transparent_45%),linear-gradient(180deg,#06101d_0%,#07192d_55%,#071523_100%)]' : 'bg-[linear-gradient(145deg,#050914_0%,#07182d_50%,#0b2038_100%)]'}`} />
