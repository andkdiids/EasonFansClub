  ...
  return <div className="relative">
    ...
    <div className={compact ? 'space-y-8' : 'space-y-12'}>
      {tours.map((tour) => <article key={tour.id} className="relative grid grid-cols-[56px_minmax(0,1fr)] gap-4 sm:grid-cols-[156px_minmax(0,1fr)] sm:gap-7">
        ...
        <Link href={`/music/concerts/${tour.id}`} className={`group overflow-hidden rounded-[26px] border border-white/10 bg-white/[0.055] transition hover:-translate-y-1 hover:border-sky-300/30 hover:bg-white/[0.09] ${compact ? 'grid sm:grid-cols-[150px_1fr]' : 'grid md:grid-cols-[230px_1fr]'}`}>
