...
type Result = {
  ...
  songs: Array<{ id: string; type: 'song'; title: string; artist: string; releaseYear: number; lyricist?: string | null; composer?: string | null; arranger?: string | null; lyricSnippet?: string | null; album: { name: string; artist: string } }>
  ...
  concerts: Array<{ id: string; type: 'concert'; title?: string | null; concertDate: string; city: string; venue?: string | null; tour: { id: string; name: string } }>
}
...
export function MusicSearchDialog({ variant = 'default', label = '搜索专辑、歌曲、现场' }: Readonly<MusicSearchDialogProps>) {
  ...
  const dialog = mounted && open ? createPortal(<div role="dialog" aria-modal="true" aria-label="搜索音乐资料" className="fixed inset-0 z-[var(--layer-dialog)] grid place-items-center bg-[rgba(3,10,20,.72)] p-4 backdrop-blur-[18px]" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false) }}>
    <div className="max-h-[84vh] w-[90%] max-w-[720px] overflow-y-auto rounded-[26px] border border-white/[0.12] bg-[#07182d]/90 p-5 text-white shadow-[0_28px_90px_rgba(0,0,0,.42)] backdrop-blur-2xl sm:p-7">
      ...
      <section><h3 className="text-xs font-black tracking-[0.18em] text-sky-300/65">巡演</h3><div className="mt-3 grid gap-2">{result.tours.map((tour) => <Link key={tour.id} href={`/music/live/tours/${tour.id}`} className="block rounded-2xl border border-white/[0.08] bg-white/[0.05] p-3 transition hover:bg-white/[0.1]"><span className="block break-words font-black text-white">{tour.name}</span><span className="mt-1 block text-xs font-bold text-slate-300/55">巡演 · {tour.startDate?.slice(0, 10) || '时间待整理'} · {tour.concertCount} 场</span></Link>)}{result.tours.length === 0 ? <p className="text-sm font-bold text-slate-400">没有匹配巡演</p> : null}</div></section>
      <section><h3 className="text-xs font-black tracking-[0.18em] text-sky-300/65">演唱会</h3><div className="mt-3 grid gap-2">{result.concerts.map((concert) => <Link key={concert.id} href={`/music/live/concerts/${concert.id}`} className="block rounded-2xl border border-white/[0.08] bg-white/[0.05] p-3 transition hover:bg-white/[0.1]"><span className="block break-words font-black text-white">{concert.title || `${concert.city} · ${concert.concertDate.slice(0, 10)}`}</span><span className="mt-1 block break-words text-xs font-bold text-slate-300/55">演唱会 · {concert.venue || '场馆待整理'} · {concert.tour.name}</span></Link>)}{result.concerts.length === 0 ? <p className="text-sm font-bold text-slate-400">没有匹配演唱会</p> : null}</div></section>
      <section><h3 className="text-xs font-black tracking-[0.18em] text-sky-300/65">歌曲</h3><div className="mt-3 grid gap-2">{result.songs.map((song) => <Link key={song.id} href={`/music/song/${song.id}`} className="flex items-start gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.05] p-3 transition hover:bg-white/[0.1]"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-white/[0.08] text-xl text-sky-200/60">♪</span><span className="min-w-0 flex-1"><span className="block truncate font-black text-white">{song.title}</span><span className="mt-1 block truncate text-xs font-bold text-slate-300/55">歌曲 · {song.album.name} · {song.artist || song.album.artist} · {song.releaseYear}</span>{song.lyricSnippet ? <span className="mt-2 block line-clamp-2 text-xs font-medium leading-5 text-sky-100/70">歌词命中：{song.lyricSnippet}</span> : null}</span></Link>)}{result.songs.length === 0 ? <p className="text-sm font-bold text-slate-400">没有匹配歌曲</p> : null}</div></section></div> : null}
    </div>
