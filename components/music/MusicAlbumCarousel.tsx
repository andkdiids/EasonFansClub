...
export function MusicAlbumCarousel({ albums }: Readonly<{ albums: MusicCarouselAlbum[] }>) {
  ...
  return <section aria-label="专辑互动展示" tabIndex={interactionPaused ? -1 : 0} onKeyDown={(event) => { if (event.key === 'ArrowLeft') move(-1); if (event.key === 'ArrowRight') move(1) }} onWheel={onWheel} className="relative isolate z-0 mx-auto flex w-full max-w-7xl flex-col items-center outline-none focus-visible:ring-4 focus-visible:ring-sky-300/50">
