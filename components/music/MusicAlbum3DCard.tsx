...
type MusicAlbumCardProps = { album: MusicCarouselAlbum; offset: number; spacing: number; cardWidth: number; selected: boolean; disabled?: boolean; onActivate: () => void; cardRef: (element: HTMLDivElement | null) => void }

export function MusicAlbum3DCard({ album, offset, spacing, cardWidth, selected, disabled = false, onActivate, cardRef }: Readonly<MusicAlbumCardProps>) {
  ...
  const [initialStyle] = useState<CSSProperties>(() => {
    ...
    const opacityValue = 1 - 0.48 * blend
    return {
      transform: `translate3d(calc(-50% + ${offset * spacing}px), ${-3 + 6 * blend}px, 0) scale(${1 - 0.12 * blend})`,
      opacity: opacityValue,
      zIndex: selected ? 30 : Math.max(1, 12 - distance),
      pointerEvents: opacityValue < 0.05 ? 'none' : undefined,
    }
  })
  return <div ref={cardRef} data-carousel-state={selected ? 'active' : distance <= 2 ? 'adjacent' : 'hidden'} className="absolute left-1/2 top-2" style={{ width: cardWidth, ...initialStyle, willChange: 'transform, opacity', backfaceVisibility: 'hidden' }}>
    <button
      type="button"
      disabled={disabled}
      aria-label={selected ? `打开专辑《${album.name}》` : `选择专辑《${album.name}》`}
      onClick={onActivate}
      className={`w-full text-left outline-none transition-transform duration-300 disabled:pointer-events-none focus-visible:rounded-[24px] focus-visible:ring-4 focus-visible:ring-sky-300/60 ${selected ? 'hover:-translate-y-1 hover:scale-[1.03]' : 'hover:-translate-y-2.5 hover:scale-[1.023]'}`}
    >
      <span className={`relative block aspect-square overflow-hidden rounded-[24px] border transition-shadow duration-300 ${selected ? 'border-white/[0.12] shadow-[0_20px_55px_rgba(35,145,230,.2)]' : 'border-white/[0.08] shadow-[0_14px_35px_rgba(2,12,27,.25)]'}`}>
        <Image src={album.coverUrl} alt={`${album.name}专辑封面`} fill sizes="(max-width:767px) 62vw, 220px" loading="lazy" draggable={false} className="object-cover" />
      </span>
