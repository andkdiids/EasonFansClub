...
export function MusicAlbum3DCard({ album, carouselIndex, offset, spacing, cardWidth, selected, disabled = false, onActivate, cardRef }: Readonly<MusicAlbumCardProps>) {
  ...
  return <div ref={cardRef} data-carousel-index={carouselIndex} data-carousel-state={selected ? 'active' : distance <= 2 ? 'adjacent' : 'hidden'} className="absolute left-1/2 top-2" style={{ width: cardWidth, ...initialStyle, willChange: 'transform, opacity', backfaceVisibility: 'hidden' }}>
    <button
      type="button"
      disabled={disabled}
    ...
    >
      <span className={`relative block aspect-square overflow-hidden rounded-[24px] border transition-shadow duration-300 ${selected ? 'border-sky-200/20 shadow-[0_16px_42px_rgba(35,145,230,.24)]' : 'border-white/[0.12] shadow-[0_10px_28px_rgba(2,12,27,.2)]'}`}>
