...
export function AttendancePanel({ concertId, loggedIn, initialAttendance }: Readonly<{
  ...
  return <section className="mt-6 border border-sky-300/20 bg-sky-300/[0.06] p-4 sm:p-5" aria-label="我的观演记录">
    ...
    {open ? <div className="fixed inset-0 z-[95] flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-5" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) requestClose() }}>
      <div role="dialog" aria-modal="true" aria-labelledby="attendance-dialog-title" className="max-h-[calc(100dvh-24px)] w-full max-w-xl overflow-y-auto border border-sky-200/20 bg-[#07182d] p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] shadow-2xl sm:p-7">
        ...
        <div className="mt-6 space-y-4">
          ...
          <label className="flex items-start gap-3 border border-white/10 p-3 text-sm font-bold text-slate-200"><input type="checkbox" checked={form.isPublic} onChange={(event) => setForm({ ...form, isPublic: event.target.checked })} className="mt-1" /><span>允许在个人主页展示<span className="mt-1 block text-xs font-medium text-slate-400">仅展示场次、城市、场馆、巡演与心情；个人笔记始终不会公开。</span></span></label>
        </div>
