...
import type { MyLivePhotoView } from '@/lib/my-live-photo-types'
...
function PublicImage({ src, ...props }: ComponentProps<typeof NextImage>) {
  const normalizedSrc = typeof src === 'string' ? publicImageVariantUrl(src, 'thumb-md') || src : src
...
export function MyLiveDashboard({ data, batchTourId, watermarkPreview }: Readonly<{ data: DashboardData; batchTourId?: string; watermarkPreview?: string }>) {
...
function ConcertCard({ record, onRemove, watermarkPreview, onPhotosChange }: { record: RecordItem; onRemove: (record: RecordItem) => void; watermarkPreview?: string; onPhotosChange: (photos: MyLivePhotoView[]) => void }) {
  const concert = record.concert!
  ...
  return <article className="my-live-concert-card min-w-0 border border-white/10 bg-white/[0.04] p-4">
    ...
    <div className="my-live-concert-card-body">
      ...
      <MyLivePhotoPanel attendanceId={record.id} photos={record.photos} manage watermarkPreview={watermarkPreview} onPhotosChange={onPhotosChange} />
    </div>
  </article>
}
