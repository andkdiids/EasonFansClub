...
export function MyLiveDashboard({ data, batchTourId }: Readonly<{ data: DashboardData; batchTourId?: string }>) {
  ...
  return <div className="my-live-dashboard">
    ...
    <header className="my-live-heading py-10 sm:py-14"><h1 className="text-5xl font-black text-white sm:text-7xl">My Live</h1><p className="mt-3 text-xl font-black text-slate-200/90">我的现场</p><p className="mt-4 text-sm font-bold text-slate-300/70">记录与你在现场相遇的每一次</p></header>
    ...
    {tab === 'overview' ? <div className="mt-8 space-y-12">
      ...
      ].map(([label,value]) => <div key={String(label)} className="min-w-0 border-b border-r border-white/10 bg-white/[0.035] p-4 sm:p-5"><dt className="break-words text-xs font-bold text-slate-400">{label}</dt><dd className="text-3xl font-black text-white sm:text-4xl">{value}</dd></div>)}</dl>
