...
export function MyLiveDashboard({ data }: Readonly<{ data: DashboardData }>) {
  ...
  return <>
    <div className="flex flex-wrap items-center justify-between gap-4"><Link href="/music/concerts" className="text-sm font-black text-sky-300/80">← 返回 Eason in Concert</Link><Link href="/music/concerts" className="border border-white/15 px-4 py-2 text-sm font-black text-white">浏览演唱会档案</Link></div>
    <header className="py-10 sm:py-14"><p className="text-xs font-black tracking-[0.22em] text-sky-300/65">MY LIVE HISTORY</p><h1 className="mt-4 text-5xl font-black text-white sm:text-7xl">我的现场</h1><p className="mt-4 text-sm font-bold text-slate-300/70">记录与你在现场相遇的每一次</p></header>
    <nav aria-label="我的现场内容" className="flex overflow-x-auto border-y border-white/10">{[['overview','总览'],['concerts','看过的场次'],['songs','歌曲图鉴'],['timeline','时间线']].map(([key,label]) => <button key={key} type="button" onClick={() => setTab(key as typeof tab)} className={tabClass(tab === key)}>{label}</button>)}</nav>
