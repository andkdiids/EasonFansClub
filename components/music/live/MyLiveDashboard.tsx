...
function ConcertCard({ record, onRemove }: { record: RecordItem; onRemove: (record: RecordItem) => void }) {
  ...
  return <article className="my-live-concert-card min-w-0 border border-white/10 bg-white/[0.04] p-4">
    <Link href={concertHref} className="my-live-concert-poster relative block aspect-[3/4] overflow-hidden bg-[#0b2038]" aria-label={`${concert.city}现场海报`}>
      {concert.posterUrl ? <Image src={concert.posterUrl} alt={`${concert.city}现场海报`} fill sizes="(max-width: 767px) 50vw, 0px" className="object-contain" /> : <span className="grid h-full place-items-center text-3xl text-sky-200/25">LIVE</span>}
    </Link>
