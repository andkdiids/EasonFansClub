...
export function MyLiveDashboard({ data }: Readonly<{ data: DashboardData }>) {
  ...
  return <>
    <div className="flex flex-wrap items-center justify-between gap-4"><Link href="/music/live" className="text-sm font-black text-sky-300/80">← 返回 Eason现场</Link><Link href="/music/live" className="border border-white/15 px-4 py-2 text-sm font-black text-white">浏览Eason现场</Link></div>
    ...
    {tab === 'overview' ? <div className="mt-8 space-y-12">
      ...
      {!availableRecords.length ? <section className="border border-white/10 bg-white/[0.04] p-7 sm:p-10"><h2 className="text-2xl font-black text-white">还没有记录看过的演唱会</h2><Link href="/music/live" className="mt-5 inline-flex bg-sky-100 px-5 py-3 text-sm font-black text-[#06101d]">浏览Eason现场</Link></section> : <>
