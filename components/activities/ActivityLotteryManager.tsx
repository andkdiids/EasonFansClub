...
export function ActivityLotteryManager({ activityId, activityTitle, registrationEndAt }: Readonly<{ activityId: string; activityTitle: string; registrationEndAt: string | null }>) {
  ...
  return <section className="mt-5 rounded-2xl border border-violet-200 bg-violet-50/60 p-4 dark:border-violet-900/60 dark:bg-violet-950/20 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black tracking-[0.18em] text-violet-700 dark:text-violet-300">活动抽奖</p><h3 className="mt-1 text-xl font-black text-brand-950 dark:text-slate-100">{activityTitle}</h3><p className="mt-1 text-xs font-bold leading-5 text-slate-600 dark:text-slate-300">抽奖资格自动继承有效活动报名，不单独配置参与上限。报名结束：{formatDate(registrationEndAt)}</p></div><button type="button" onClick={() => setExpanded((value) => !value)} className="min-h-10 rounded-full bg-violet-700 px-4 py-2 text-sm font-black text-white">{expanded ? '收起设置' : '新建抽奖'}</button></div>
