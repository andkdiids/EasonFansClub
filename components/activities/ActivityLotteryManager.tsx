        ...
        {data?.lotteries.map((lottery) => {
          ...
          return (
            <article key={lottery.id} className="rounded-xl border border-violet-200 bg-white p-4 dark:border-violet-900 dark:bg-slate-950">
              ...
              <div className="mt-3 grid gap-2 text-xs font-bold text-slate-600 dark:text-slate-300 sm:grid-cols-4"><p>有效报名：{lottery.eligibleCount ?? currentParticipants}</p><p>中奖名额：{slots} · 实际中奖：{lottery.winnerCount ?? 0}</p><p>当前理论中奖率：{currentRate.toFixed(2)}%</p><p>候选快照：{lottery.candidateSnapshotCount ? `${lottery.candidateSnapshotCount} 人` : lottery.status === 'DRAWN' ? '未记录（历史开奖）' : '开奖时记录'}</p></div>
