...
export function ActivityRegistrationManager({ activityId, activityTitle, verificationMode, onClose }: Readonly<{ activityId: string; activityTitle: string; verificationMode: 'NONE' | 'MANUAL' | 'QR'; onClose: () => void }>) {
  ...
  const verifyToken = useCallback(async (rawToken: string) => {
    ...
    try {
      ...
      setSelectedEntitlements(preview.entitlements.filter((item) => item.redeemable).map((item) => `${item.type}:${item.id}`))
    } catch (verifyError) {
  ...
  async function confirmRedemption() {
    ...
    const entitlements = redemptionPreview.entitlements
      .filter((item) => selectedEntitlements.includes(`${item.type}:${item.id}`) && item.redeemable)
  ...
  return <section className="rounded-[28px] border border-emerald-200 bg-emerald-50/50 p-5 shadow-sm dark:border-emerald-900/60 dark:bg-emerald-950/20 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black tracking-[0.18em] text-emerald-700 dark:text-emerald-300">报名管理</p><h2 className="mt-1 break-words text-2xl font-black text-brand-950 dark:text-slate-100">{activityTitle}</h2><p className="mt-2 text-sm font-bold text-slate-600 dark:text-slate-300">有效 {summary.activeCount} · 已核销 {summary.verifiedCount} · 已取消 {summary.cancelledCount}</p></div><button type="button" onClick={onClose} className="min-h-10 rounded-full border border-slate-200 px-4 py-2 text-sm font-black text-slate-600 dark:border-slate-700 dark:text-slate-300">收起</button></div>
    {activityMeta ? <div className="mt-4 grid gap-2 rounded-xl border border-emerald-200 bg-white/70 p-3 text-xs font-bold leading-5 text-slate-600 dark:border-emerald-900 dark:bg-slate-900/70 dark:text-slate-300 sm:grid-cols-2"><p>活动时间：{formatDate(activityMeta.startsAt) || '未设置'}{activityMeta.endsAt ? ` — ${formatDate(activityMeta.endsAt)}` : ''}</p><p>报名时间：{registrationWindow}</p><p>报名名额：{activityMeta.signupLimit && activityMeta.signupLimit > 0 ? `${summary.activeCount}/${activityMeta.signupLimit}` : `${summary.activeCount} 人（不限人数）`}</p><p>报名费用：{activityMeta.registrationFee > 0 ? `${activityMeta.registrationFee} 挂号费` : '免费'}</p><p>核销方式：{verificationLabel}</p>{activityMeta.reward ? <p className="sm:col-span-2">隐藏奖励：{activityMeta.reward.name} · {activityMeta.reward.code}</p> : null}</div> : null}
    <ActivityLotteryManager activityId={activityId} activityTitle={activityTitle} activityEndAt={activityMeta?.endsAt || null} />
    ...
    {redemptionPreview ? <div role="dialog" aria-modal="true" className="mt-4 rounded-2xl border-2 border-emerald-300 bg-white p-4 shadow-lg dark:border-emerald-700 dark:bg-slate-950 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><div className="size-12 shrink-0 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">{redemptionPreview.user.avatarUrl ? <img src={redemptionPreview.user.avatarUrl} alt="" className="size-full object-cover" /> : null}</div><div><h3 className="font-black text-brand-950 dark:text-slate-100">核销确认</h3><p className="mt-1 text-sm font-bold text-slate-700 dark:text-slate-200">{redemptionPreview.user.nickname} · E院ID {redemptionPreview.user.uid}</p></div></div><button type="button" onClick={() => { setRedemptionPreview(null); setSelectedEntitlements([]) }} className="min-h-9 rounded-full border border-slate-200 px-3 text-sm font-black text-slate-600 dark:border-slate-700 dark:text-slate-300">取消</button></div><p className="mt-4 text-sm font-black text-slate-700 dark:text-slate-200">活动：{redemptionPreview.activity.title}</p><div className="mt-3 space-y-2">{redemptionPreview.entitlements.map((item) => { const key = `${item.type}:${item.id}`; return <label key={key} className={`flex items-start gap-3 rounded-xl border p-3 text-sm ${item.redeemable ? 'border-sky-100 dark:border-slate-700' : 'border-slate-100 bg-slate-50 text-slate-400 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-500'}`}><input type="checkbox" checked={selectedEntitlements.includes(key)} disabled={!item.redeemable || busyId === 'qr-confirm'} onChange={() => setSelectedEntitlements((current) => current.includes(key) ? current.filter((value) => value !== key) : [...current, key])} className="mt-1 size-4" /><span className="min-w-0"><span className="block font-black">{item.title} ×{item.quantity}</span>{item.subtitle ? <span className="mt-1 block text-xs font-bold">{item.subtitle}</span> : null}{item.type === 'LOTTERY_PRIZE' && item.redemptionState === 'REDEEMED' ? <span className="mt-1 block text-xs font-black text-emerald-600">已兑奖{item.redeemedAt ? ` · ${formatDate(item.redeemedAt)}` : ''}</span> : item.type === 'LOTTERY_PRIZE' && item.redemptionState === 'EXPIRED' ? <span className="mt-1 block text-xs font-black text-rose-600">已失效</span> : item.type === 'LOTTERY_PRIZE' && item.redemptionState === 'WAITING_FOR_CHECK_IN' ? <span className="mt-1 block text-xs font-black text-amber-600">待核销后兑奖</span> : item.type === 'LOTTERY_PRIZE' && item.redemptionState === 'REDEEMABLE' ? <span className="mt-1 block text-xs font-black text-sky-600">可兑奖</span> : item.status === 'REDEEMED' ? <span className="mt-1 block text-xs font-black text-emerald-600">已核销{item.redeemedAt ? ` · ${formatDate(item.redeemedAt)}` : ''}</span> : item.redeemable ? <span className="mt-1 block text-xs font-black text-sky-600">待核销</span> : <span className="mt-1 block text-xs font-black">当前不可核销</span>}</span></label> })}</div>{!redemptionPreview.entitlements.some((item) => item.redeemable) ? <p className="mt-3 rounded-lg bg-slate-100 px-3 py-2 text-sm font-black text-slate-600 dark:bg-slate-900 dark:text-slate-300">当前没有待核销或可兑奖项目。</p> : null}<button type="button" onClick={() => void confirmRedemption()} disabled={!redemptionPreview.entitlements.some((item) => item.redeemable) || busyId === 'qr-confirm'} className="mt-4 min-h-11 w-full rounded-xl bg-emerald-700 px-4 py-2 text-sm font-black text-white disabled:opacity-50">{busyId === 'qr-confirm' ? '核销中…' : '确认核销所选项目'}</button></div> : null}
    <div className="mt-4 space-y-3">{loading && !rows.length ? <p className="py-6 text-center text-sm font-bold text-slate-500">加载中…</p> : rows.map((registration) => <article key={registration.id} className="rounded-xl border border-sky-100 bg-white p-4 dark:border-slate-700 dark:bg-slate-900"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h3 className="break-words font-black text-brand-950 dark:text-slate-100">{registration.User.nickname} · E院ID {registration.User.uid}</h3><p className="mt-1 text-xs font-bold text-slate-500">报名于 {new Date(registration.registeredAt).toLocaleString('zh-CN', { hour12: false })}{registration.cancelledAt ? ` · 取消于 ${new Date(registration.cancelledAt).toLocaleString('zh-CN', { hour12: false })}` : ''}{registration.checkedInAt ? ` · 核销于 ${new Date(registration.checkedInAt).toLocaleString('zh-CN', { hour12: false })}` : ''}</p></div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${registration.status === 'CANCELLED' ? 'bg-slate-100 text-slate-500' : registration.verifiedAt && registration.checkInSource === 'AUTO_AFTER_ACTIVITY_END' ? 'bg-amber-100 text-amber-700' : registration.verifiedAt ? 'bg-emerald-100 text-emerald-700' : 'bg-sky-100 text-sky-700'}`}>{registration.status === 'CANCELLED' ? '已取消' : registration.verifiedAt && registration.checkInSource === 'AUTO_AFTER_ACTIVITY_END' ? '活动结束自动核销' : registration.verifiedAt ? `已核销（${registration.verificationMethod || '—'}）` : '有效报名'}</span>{verificationMode === 'MANUAL' && registration.status === 'ACTIVE' && !registration.verifiedAt ? <button type="button" onClick={() => void verifyRegistration(registration.id)} disabled={Boolean(busyId)} className="min-h-9 rounded-full bg-emerald-700 px-3 text-xs font-black text-white disabled:opacity-50">{busyId === registration.id ? '核销中…' : '手动核销'}</button> : null}</div></div><div className="mt-3 grid gap-2 border-t border-sky-100 pt-3 text-xs font-bold text-slate-600 dark:border-slate-700 dark:text-slate-300 sm:grid-cols-2"><p>报名费用：{registration.paidRegistrationFee} 挂号费</p>{registration.linkedMaterialRedemption ? <p>自动兑换物料：{registration.linkedMaterialRedemption.title} · {registration.linkedMaterialRedemption.status === 'REDEEMED' ? '已核销' : registration.linkedMaterialRedemption.status === 'CANCELLED' ? '兑换已取消' : '待核销'}<br />{registration.linkedMaterialRedemption.redeemCode} · {checkInSourceLabel(registration.checkInSource)}</p> : null}</div>{registration.answers.length ? <dl className="mt-3 grid gap-2 border-t border-sky-100 pt-3 sm:grid-cols-2 dark:border-slate-700">{registration.answers.map((answer) => <div key={answer.questionId} className="min-w-0 text-sm"><dt className="font-black text-slate-500">{answer.questionTitle}</dt><dd className="mt-1 break-words font-bold text-slate-800 dark:text-slate-200">{Array.isArray(answer.value) ? answer.value.join('、') : answer.value}</dd></div>)}</dl> : <p className="mt-3 border-t border-sky-100 pt-3 text-xs font-bold text-slate-500 dark:border-slate-700">轻量确认报名，无额外问题。</p>}</article>)}{!loading && !rows.length ? <p className="py-6 text-center text-sm font-bold text-slate-500">没有匹配的报名记录。</p> : null}</div>
