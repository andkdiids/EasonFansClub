  ...
  return (
    <section>
      <div className="grid gap-3 rounded-2xl border border-sky-100 bg-white/80 p-4 shadow-sm sm:grid-cols-[minmax(0,1fr)_11rem_11rem] dark:border-slate-700 dark:bg-slate-900/80">
        <label className="text-sm font-black text-brand-950 dark:text-slate-100">搜索活动
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索标题或说明" className="mt-1 min-h-11 w-full rounded-xl border border-sky-100 bg-white px-3 font-bold text-slate-700 outline-none focus:border-sky-400 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100" />
        </label>
        <label className="text-sm font-black text-brand-950 dark:text-slate-100">活动状态
          <select value={status} onChange={(event) => setStatus(event.target.value as 'ALL' | ActivityDisplayStatus)} className="mt-1 min-h-11 w-full rounded-xl border border-sky-100 bg-white px-3 font-bold text-slate-700 outline-none focus:border-sky-400 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100">
        ...
        <label className="text-sm font-black text-brand-950 dark:text-slate-100">活动类型
          <select value={type} onChange={(event) => setType(event.target.value as 'ALL' | ActivityView['type'])} className="mt-1 min-h-11 w-full rounded-xl border border-sky-100 bg-white px-3 font-bold text-slate-700 outline-none focus:border-sky-400 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100">
            <option value="ALL">全部类型</option>
            {activityTypeValues.map((item) => <option key={item} value={item}>{activityTypeLabels[item]}</option>)}
          </select>
        </label>
      </div>
      <div className="mt-4 flex items-center justify-between gap-3 text-sm font-bold text-slate-500 dark:text-slate-400">
