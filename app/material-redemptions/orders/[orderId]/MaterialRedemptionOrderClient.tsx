...
export function MaterialRedemptionOrderClient({ orderId }: { orderId: string }) {
  ...
  return (
    <main className="site-page-main mx-auto min-w-0 max-w-4xl space-y-4 px-3 py-4 sm:space-y-5 sm:px-5 sm:py-9">
      ...
      <section className="border border-sky-100 bg-white/90 p-4 shadow-sm sm:p-8"><p className="text-sm font-black tracking-[0.18em] text-brand-700">兑换成功</p><h1 className="mt-2 break-words text-3xl font-black text-brand-950">{order.material.title}</h1><p className="mt-3 text-sm font-bold leading-6 text-slate-600 sm:leading-7">请在核销截止时间前向工作人员展示兑换码或二维码。扫码只会打开管理员确认页，不会自动完成核销。</p><dl className="mt-4 grid gap-2 sm:mt-6 sm:grid-cols-3 sm:gap-3"><div className="border border-sky-100 bg-sky-50/60 p-3 sm:p-4"><dt className="text-xs font-bold text-slate-500">兑换码</dt><dd className="mt-1 break-all text-lg font-black tracking-[0.08em] text-brand-950 sm:mt-2">{order.redeemCode}</dd></div><div className="border border-sky-100 bg-sky-50/60 p-3 sm:p-4"><dt className="text-xs font-bold text-slate-500">数量</dt><dd className="mt-1 text-lg font-black text-brand-950 sm:mt-2">{order.quantity} 件</dd></div><div className="border border-sky-100 bg-sky-50/60 p-3 sm:p-4"><dt className="text-xs font-bold text-slate-500">核销截止</dt><dd className="mt-1 text-sm font-black text-brand-950 sm:mt-2">{new Date(order.material.redeemEndAt).toLocaleString('zh-CN', { hour12: false, timeZone: 'Asia/Shanghai' })}</dd></div></dl></section>
