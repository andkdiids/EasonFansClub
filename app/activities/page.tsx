...
export default async function ActivitiesPage() {
  const [activities, config] = await Promise.all([
    prisma.activity.findMany({ where: { status: 'PUBLISHED' }, orderBy: { createdAt: 'desc' }, take: 12 }),
    getSiteAppearance(),
  ])
    ...
    <>
      <main className="site-page-main flat-page mx-auto max-w-7xl space-y-6 px-5 py-8">
        <section className={`relative isolate min-h-64 overflow-hidden border shadow-sm ${hasBanner ? 'border-slate-800 bg-[#071523] text-white' : 'border-sky-100 bg-white/85 text-brand-950'}`}>
          ...
          <div className="relative z-10 p-7 sm:p-10"><p className={`text-sm font-black uppercase tracking-[0.22em] ${hasBanner ? 'text-sky-200' : 'text-brand-700'}`}>Activities</p><h1 className="mt-3 text-4xl font-black">活动中心</h1><p className={`mt-4 max-w-2xl leading-8 ${hasBanner ? 'text-white/80' : 'text-slate-600'}`}>演唱会报名、聚会、线上活动和抽奖都会在这里发布。</p></div>
        </section>
        <section className="grid gap-4 md:grid-cols-2">
          {activities.length ? activities.map((item) => (
            <Link key={item.id} href={`/activities/${item.id}`} className="block rounded-2xl border border-sky-100 bg-white/80 p-5 shadow-sm">
