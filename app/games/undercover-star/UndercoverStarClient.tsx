...
type UndercoverStatsView = { totalGames: number; totalWins: number; totalLosses: number; winRate: number; civilianGames: number; civilianWins: number; undercoverGames: number; undercoverWins: number; successfulUndercoverVotes: number; undercoverSurvivalWins: number; undercoverGuessWins: number }
...
function phaseTitle(phase: UndercoverPublicMatchSnapshot['phase']) {
  return ({ ROLE_REVEAL: '查看身份', DESCRIBING: '描述阶段', VOTING: '投票阶段', TIE_VOTING: '平票加赛', UNDERCOVER_GUESS: '最后一搏', FINISHED: '本局结果' } as const)[phase]
}
...
export function UndercoverStarClient() {
  ...
  if (loading) return <main className="mx-auto max-w-6xl px-4 py-12 text-sm font-bold text-slate-500">正在加载卧底巨星…</main>

  return (
    <main className="undercover-star-page mx-auto min-h-screen max-w-6xl px-4 py-6 pb-[calc(2rem+env(safe-area-inset-bottom))] sm:px-6 lg:py-10">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4 border-b border-sky-100 pb-5"><div><Link href="/games" className="text-sm font-black text-brand-700">← 返回娱乐天空</Link><p className="mt-5 text-xs font-black tracking-[0.2em] text-brand-700">UNDERCOVER STAR</p><h1 className="mt-2 text-3xl font-black tracking-tight text-brand-950 sm:text-4xl">卧底巨星</h1><p className="mt-2 text-sm font-bold text-slate-500">谁说得最像真的，谁就最可疑。</p></div>{view !== 'LOBBY' ? <button type="button" onClick={resetToLobby} className="border border-sky-200 px-4 py-2 text-sm font-black text-brand-700">返回大厅</button> : null}</header>
