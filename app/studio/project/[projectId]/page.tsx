...
export default async function StudioPublicProjectPage({ params }: Readonly<{ params: Promise<{ projectId: string }> }>) {
  ...
  const project = await prisma.studioProject.findFirst({
    ...
    select: { id: true, title: true, description: true, data: true, thumbnailUrl: true, likeCount: true, favoriteCount: true, viewCount: true, downloadCount: true, createdAt: true, updatedAt: true, User: { select: { nickname: true, isDeleted: true, status: true } }, Artist: { select: { id: true, slug: true, name: true, avatar: true, description: true } } },
  })
