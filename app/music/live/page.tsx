...
export default async function MusicLivePage() {
  ...
  return <MusicArchiveShell backgroundVisual={config.heroVisuals.music}>
    ...
    <header className="py-12 sm:py-16"><h1 className="text-5xl font-black tracking-tight text-white sm:text-7xl">Eason in Concert</h1><p className="mt-5 max-w-3xl text-sm font-bold leading-7 text-slate-300/70 sm:text-base">收录陈奕迅巡演、演唱会场次、现场歌单与特别时刻</p></header>
    ...
    {!tours.length ? <section className="border border-white/10 bg-white/[0.055] p-8 sm:p-12"><h2 className="text-3xl font-black text-white">现场档案正在整理中</h2><p className="mt-3 text-sm font-bold text-slate-300/65">已发布巡演将在这里出现。</p></section> : <>
      <section aria-labelledby="live-tours-title"><h2 id="live-tours-title" className="text-3xl font-black text-white sm:text-4xl">巡演档案</h2><div className="mt-7 grid min-w-0 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">{resolvedTours.map((tour) => {
