...
export default async function MusicLivePage() {
  const [tours, latestConcerts, config] = await Promise.all([
    prisma.musicTour.findMany({
      ...
      take: 50,
      select: { id: true, name: true, subtitle: true, posterUrl: true, startDate: true, endDate: true, category: true, MusicConcert: { where: { status: 'PUBLISHED' }, orderBy: [{ concertDate: 'asc' }, { createdAt: 'asc' }, { id: 'asc' }], select: { city: true, posterUrl: true } }, _count: { select: { MusicConcert: { where: { status: 'PUBLISHED' } } } } },
    }),
  ...
  return <MusicArchiveShell backgroundVisual={config.heroVisuals.music}>
    ...
    {!tours.length ? <section className="border border-white/10 bg-white/[0.055] p-8 sm:p-12"><h2 className="text-3xl font-black text-white">现场档案正在整理中</h2><p className="mt-3 text-sm font-bold text-slate-300/65">已发布巡演将在这里出现。</p></section> : <>
      <section aria-labelledby="live-tours-title"><p className="text-xs font-black tracking-[0.2em] text-sky-300/65">TOUR ARCHIVE</p><h2 id="live-tours-title" className="mt-2 text-3xl font-black text-white sm:text-4xl">巡演档案</h2><div className="mt-7 grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{resolvedTours.map((tour) => <Link key={tour.id} href={`/music/live/tours/${generateArchiveSlug(tour.name)}`} className="group min-w-0 overflow-hidden border border-white/10 bg-white/[0.055] transition hover:border-sky-300/30 hover:bg-white/[0.09]"><div className="relative aspect-square bg-[#0b2038]"><ConcertCover resolvedPosterUrl={tour.resolvedPosterUrl} alt={`${tour.name}巡演海报`} sizes="(max-width: 640px) 50vw, 25vw" /></div><div className="p-5"><h3 className="break-words text-xl font-black text-white">{tour.name}</h3>{tour.subtitle ? <p className="mt-2 break-words text-sm font-bold text-slate-300/65">{tour.subtitle}</p> : null}<p className="mt-4 text-xs font-black text-sky-200/60">{formatLiveDateRange(tour.startDate, tour.endDate)}</p><p className="mt-2 text-xs font-bold text-slate-300/55">{tour.concertCount} 场 · {tour.cityCount} 个城市</p></div></Link>)}</div></section>
