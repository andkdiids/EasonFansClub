...
export default async function MusicConcertBySlugPage({
  ...
  return <MusicArchiveShell maxWidth="max-w-6xl" backgroundVisual={config.heroVisuals.music}>
    ...
    {attendance ? <MyLivePhotoPanel attendanceId={attendance.id} photos={serializeMyLivePhotos(attendance.MyLivePhoto)} manage watermarkPreview={`${currentUser?.nickname || 'E院用户'}  UID:${currentUser?.uid || ''}`} /> : null}
    {setlist.all.length ? (
      <>
