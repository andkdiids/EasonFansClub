...
export default async function MusicConcertPage({ params }: { params: Promise<{ concertId: string }> }) {
  ...
  const [concert, config, attendance] = await Promise.all([
    ...
    currentUser ? prisma.userMusicConcert.findUnique({
      where: { userId_concertId: { userId: currentUser.id, concertId } },
  ...
  return <MusicArchiveShell maxWidth="max-w-6xl" backgroundVisual={config.heroVisuals.music}>
    <div className="flex flex-wrap items-center gap-4"><BackButton fallbackHref={`/music/live/tours/${generateArchiveSlug(concert.MusicTour.name)}`} label="返回上一页" /><Link href={`/music/live/tours/${generateArchiveSlug(concert.MusicTour.name)}`} className="text-sm font-black text-sky-300/80">返回巡演：{concert.MusicTour.name}</Link></div>
    ...
    {concert.MusicConcertSetlistItem.length ? <SetlistBlock items={concert.MusicConcertSetlistItem} title="现场歌单" /> : null}
