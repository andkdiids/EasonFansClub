...
export default async function MusicAlbumsPage() {
  const [albums, config] = await Promise.all([
    prisma.musicAlbum.findMany({ where: { status: 'PUBLISHED' }, orderBy: [{ displayOrder: 'asc' }, { releaseYear: 'desc' }, { createdAt: 'desc' }], include: { _count: { select: { songs: true } } } }),
    getSiteAppearance(),
  ])
