...
export default async function MusicAlbumsPage() {
  ...
  return (
    <MusicArchiveShell maxWidth="max-w-6xl" backgroundVisual={config.heroVisuals.music}>
      ...
      <section className="mt-8"><h1 className="text-4xl font-black tracking-tight text-white sm:text-6xl">专辑墙</h1><p className="mt-4 max-w-2xl text-sm font-bold leading-7 text-slate-300/65">按发行年份浏览陈奕迅音乐专辑资料。</p></section>
      {albums.length > 0 ? <section className="mt-8 grid min-w-0 grid-cols-3 gap-x-3 gap-y-5 sm:mt-10 sm:grid-cols-3 sm:gap-x-4 sm:gap-y-8 md:grid-cols-4 xl:grid-cols-5">{albums.map((album) => <MusicAlbumCard key={album.id} theme="dark" album={{ ...album, songCount: album._count.MusicSong }} />)}</section> : <p className="mt-9 rounded-[26px] border border-white/10 bg-white/[0.05] p-8 text-sm font-bold text-slate-300/65">专辑墙正在建设中。</p>}
    </MusicArchiveShell>
  )
}
