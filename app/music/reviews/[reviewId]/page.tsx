...
export default async function AlbumReviewDetailPage({ params }: Readonly<{ params: Promise<{ reviewId: string }> }>) {
  ...
  return <MusicArchiveShell maxWidth="max-w-5xl" backgroundVisual={config.heroVisuals.music}>
    ...
    <article className="mt-10 overflow-hidden rounded-[32px] border border-white/10 bg-[#08192b]/80 shadow-[0_30px_90px_rgba(0,0,0,.28)] backdrop-blur-xl">
      {(review.coverUrl || review.MusicAlbum.coverUrl) ? <div className="relative aspect-[16/8]"><Image src={(review.coverUrl || review.MusicAlbum.coverUrl)!} alt={review.title} fill priority sizes="(max-width: 1024px) 100vw, 960px" className="object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#08192b] via-transparent to-transparent" /></div> : null}
      <div className="p-6 sm:p-10">
        ...
        {images.length ? <div className="mt-10 grid gap-5">{images.map((url, index) => <figure key={url} className="relative aspect-[16/10] overflow-hidden rounded-[24px] border border-white/10 bg-[#0b2038]"><Image src={url} alt={`${review.title} 资料图片 ${index + 1}`} fill sizes="(max-width: 1024px) 100vw, 900px" className="object-contain" /></figure>)}</div> : null}
        <aside className="mt-12 grid gap-5 rounded-[24px] border border-sky-300/15 bg-sky-300/[0.06] p-5 sm:grid-cols-[112px_1fr] sm:items-center">
          ...
          <div><p className="text-xs font-black tracking-[0.18em] text-sky-300/60">RELATED ALBUM</p><h2 className="mt-2 text-2xl font-black text-white">《{review.MusicAlbum.name}》</h2><p className="mt-2 text-sm font-bold text-slate-300/60">{review.MusicAlbum.releaseYear} · {review.MusicAlbum.language} · {review.MusicAlbum._count.MusicSong} 首歌曲</p><Link href={`/music/album/${review.MusicAlbum.id}`} className="mt-4 inline-flex text-sm font-black text-sky-200">查看专辑档案 →</Link></div>
        </aside>
      </div>
    </article>
  </MusicArchiveShell>
