...
export function FeedbackCenter({ initialFeedbackId, initialFocusId, initialTab = 'feedback' }: { initialFeedbackId?: string; initialFocusId?: string; initialTab?: 'feedback' | 'changelog' }) {
  ...
  async function loadFeedbacks() {
    ...
    try {
      ...
      const data = await requestJson(`/api/feedback?${params.toString()}`)
        ...
        <section className="grid gap-5 lg:grid-cols-[360px_1fr]">
          <div className="space-y-4">
            ...
            <div className="rounded-[24px] border border-sky-100 bg-white/88 p-3 shadow-sm">
              ...
              {!loading && !feedbacks.length ? <p className="rounded-2xl bg-sky-50 p-4 text-sm font-bold text-slate-500">还没有反馈。遇到问题或有好点子时，可以从上方提交给管理员。</p> : null}
              {feedbacks.map((item) => (
                <button key={item.id} onClick={() => setSelectedId(item.id)} className={`mb-2 w-full rounded-2xl p-3 text-left transition ${selectedId === item.id ? 'bg-sky-100' : 'bg-sky-50/70 hover:bg-sky-50'}`}>
                  ...
                  <p className="mt-1 text-xs font-bold text-slate-400">创建：{formatTime(item.createdAt)}</p>
