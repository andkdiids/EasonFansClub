  ...
  return (
    <section className="rounded-[28px] border border-sky-100 bg-white/90 p-5 shadow-sm sm:p-7">
      ...
      <form onSubmit={applyFilters} className="mt-6 grid gap-3 rounded-2xl border border-sky-100 bg-sky-50/50 p-4 md:grid-cols-2 xl:grid-cols-4">
        <label className="grid gap-1 text-xs font-black text-slate-600"><span>管理员 / UID</span><input value={admin} onChange={(event) => setAdmin(event.target.value)} placeholder="昵称、内部账号或 UID" className="min-h-10 rounded-xl border border-sky-100 bg-white px-3" /></label>
        ...
        <label className="grid gap-1 text-xs font-black text-slate-600"><span>对象类型</span><select value={targetType} onChange={(event) => setTargetType(event.target.value)} className="min-h-10 rounded-xl border border-sky-100 bg-white px-3"><option value="">全部对象</option><option value="POST">帖子</option><option value="ACTIVITY">活动</option></select></label>
        <label className="grid gap-1 text-xs font-black text-slate-600"><span>目标搜索</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="帖子或活动标题、ID、作者 UID" className="min-h-10 rounded-xl border border-sky-100 bg-white px-3" /></label>
