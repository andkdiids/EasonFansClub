...
export function ReviewCenter({ initialType }: Readonly<{ initialType: string }>) {
  ...
  const load = useCallback(async (nextType = type, nextStatus = status, nextKeyword = keyword) => {
    setLoading(true)
    setError('')
    try {
  ...
  return <main className="admin-mobile-page mx-auto max-w-7xl space-y-5 px-4 py-7 sm:px-5 sm:py-9">
    <section className="border border-sky-100 bg-white/95 p-5 shadow-sm sm:p-8">
      ...
      <div className="mt-6 flex flex-wrap gap-2" role="tablist" aria-label="审核类型">
        <button type="button" role="tab" aria-selected={type === 'ALL'} onClick={() => setType('ALL')} className={`rounded-full px-4 py-2 text-sm font-black ${type === 'ALL' ? 'bg-brand-950 text-white' : 'bg-sky-50 text-brand-700'}`}>全部 <span className="ml-1 text-xs opacity-70">{counts?.total || 0}</span></button>
        {typeTabs.map((tab) => <button type="button" role="tab" key={tab.type} aria-selected={type === tab.type} onClick={() => setType(tab.type)} className={`rounded-full px-4 py-2 text-sm font-black ${type === tab.type ? 'bg-brand-950 text-white' : 'bg-sky-50 text-brand-700'}`}>{tab.label} <span className="ml-1 text-xs opacity-70">{tab.pending}</span></button>)}
      </div>

      <div className="mt-3 flex flex-wrap gap-2" role="tablist" aria-label="审核状态">
        {(['ALL', 'PENDING', 'APPROVED', 'REJECTED'] as Filter[]).map((value) => <button type="button" role="tab" key={value} aria-selected={status === value} onClick={() => setStatus(value)} className={`rounded-full px-3 py-1.5 text-xs font-black ${status === value ? 'bg-sky-200 text-brand-950' : 'bg-slate-50 text-slate-600'}`}>{statusLabels[value]}{value === 'PENDING' ? ` ${counts?.pending || 0}` : ''}</button>)}
      </div>

      <div className="mt-4 flex flex-wrap gap-2">
        <input type="search" value={searchInput} onChange={(event) => setSearchInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') setKeyword(searchInput.trim()) }} placeholder="搜索标题 / 作者 / UID / 内容摘要" className="min-h-10 min-w-[220px] flex-1 border border-sky-100 bg-white px-3 text-sm font-bold text-brand-950 outline-none focus:border-brand-500" />
        <button type="button" onClick={() => setKeyword(searchInput.trim())} disabled={loading} className="min-h-10 bg-brand-950 px-4 text-sm font-black text-white disabled:opacity-50">搜索</button>
        {keyword ? <button type="button" onClick={() => { setSearchInput(''); setKeyword('') }} className="min-h-10 bg-sky-50 px-4 text-sm font-black text-brand-700">清除</button> : null}
      </div>
    </section>
...
function ReviewCard({ item, busy, selectedBoardId, onBoardChange, onApprove, onReject, onDelete }: Readonly<{
  item: ReviewItem
  busy: boolean
  ...
  return <article className="grid gap-4 border border-sky-100 bg-white/95 p-4 shadow-sm sm:p-5 lg:grid-cols-[96px_minmax(0,1fr)_auto] lg:items-start">
    <div className="flex h-24 w-24 items-center justify-center overflow-hidden border border-sky-100 bg-sky-50/50">
      {item.cover ? <img src={item.cover} alt={`${item.title}封面`} className="h-full w-full object-cover" /> : <span className="px-2 text-center text-xs font-black text-sky-700">{item.sourceType === 'POST' ? '帖子' : item.sourceType === 'SALON' ? '沙龙' : '审核项'}</span>}
    </div>
    <div className="min-w-0">
      ...
      {mediaItems.length ? <div className="mt-4 flex flex-wrap gap-3" aria-label={`帖子图片，共 ${mediaItems.length} 张`}>{mediaItems.map((media, index) => <ImageViewer key={media.id} src={media.src} previewSrc={media.previewSrc} alt={media.alt} gallery={mediaItems} initialIndex={index} imageClassName="h-28 w-40 rounded-xl object-cover" buttonClassName="block h-28 w-40 cursor-zoom-in overflow-hidden rounded-xl bg-slate-100 text-left" />)}</div> : null}
    </div>
