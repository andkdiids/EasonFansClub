...
function ReviewCard({ item, busy, focused, selectedBoardId, onBoardChange, onApprove, onReject, onDelete }: Readonly<{
  ...
  return <article data-review-source={item.sourceType} data-review-target={item.sourceId} className={`grid gap-4 border border-sky-100 bg-white/95 p-4 shadow-sm sm:p-5 ${showMediaColumn ? 'lg:grid-cols-[96px_minmax(0,1fr)_auto]' : 'lg:grid-cols-[minmax(0,1fr)_auto]'} lg:items-start ${focused ? 'ring-2 ring-amber-300 ring-offset-2' : ''}`}>
    ...
    <div className="min-w-0">
      <div className="flex flex-wrap items-center gap-2"><span className="bg-sky-50 px-2 py-1 text-[10px] font-black text-brand-700">{reviewSourceLabel(item.sourceType)}</span>{item.sourceType === 'SALON' ? <span className="bg-violet-50 px-2 py-1 text-[10px] font-black text-violet-700">类型：{item.reviewKind === 'EDIT' ? '编辑' : '投稿'}</span> : null}<span className={`px-2 py-1 text-[10px] font-black ${item.status === 'PENDING' ? 'bg-amber-50 text-amber-800' : item.status === 'APPROVED' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-800'}`}>{statusLabel(item.status)}</span><h3 className="min-w-0 break-words text-lg font-black text-brand-950">{item.title}</h3></div>
