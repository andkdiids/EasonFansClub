...
export function AdminTourManager() {
  ...
  async function save(event: FormEvent) {
    ...
    const response = await fetch(editing ? `/api/admin/music/tours/${editing.id}` : '/api/admin/music/tours', { method: editing ? 'PATCH' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
    const data = await response.json().catch(() => null)
    if (!response.ok) setError(data?.message || '保存失败')
    else { setMessage(data.message); setEditing(null); setForm(empty); await load() }
    setBusy(false)
  }
  ...
  return <main className="admin-mobile-page mx-auto max-w-6xl space-y-6 px-4 py-7 sm:px-5">
    ...
    <form onSubmit={save} className="border border-sky-100 bg-white/90 p-5 shadow-sm sm:p-7">
      <div className="flex items-center justify-between gap-3"><h2 className="text-2xl font-black text-brand-950">{editing ? `编辑：${editing.name}` : '创建巡演'}</h2>{editing ? <button type="button" onClick={() => { setEditing(null); setForm(empty) }} className="text-sm font-black text-slate-500">取消编辑</button> : null}</div>
