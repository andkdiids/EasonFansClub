...
function SetlistEditor({
  ...
  return (
    <section className="border border-sky-100 bg-white/90 p-5 sm:p-7">
      ...
      <div className="mt-5 grid gap-3 border border-sky-100 bg-sky-50/50 p-4 lg:grid-cols-[minmax(0,1fr)_auto]">
      ...
      <div className="mt-5 space-y-3">
        {items.map((item, index) => (
          <article key={`${item.id || 'new'}-${item.songId || 'none'}-${index}`} draggable onDragStart={(event) => event.dataTransfer.setData('text/plain', String(index))} onDragOver={(event) => event.preventDefault()} onDrop={(event) => drop(event, index)} className="border border-sky-100 bg-white p-4 shadow-sm">
            <div className="grid min-w-0 gap-3 lg:grid-cols-[40px_130px_minmax(160px,1fr)_minmax(140px,1fr)_auto] lg:items-start">
  ...
  return <main className="admin-mobile-page mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-5">
    ...
    <form onSubmit={save} className="space-y-6">
      <section className="border border-sky-100 bg-white/90 p-5 sm:p-7"><h2 className="text-2xl font-black text-brand-950">基本信息</h2><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="text-sm font-black sm:col-span-2 lg:col-span-4">介绍<textarea value={form.description} onChange={(event) => updateForm('description', event.target.value)} className={`${field} mt-1 min-h-28`} /></label>
