...
type Concert = { id: string; tourId: string; title?: string | null; concertDate: string; city: string; countryOrRegion?: string | null; venue?: string | null; sessionNumber?: string | null; posterUrl?: string | null; description?: string | null; status: 'DRAFT' | 'PUBLISHED'; sortOrder: number; tour: Tour; setlist: SetlistItem[]; highlights: Highlight[] }
...
export function AdminConcertEditor({ concertId }: { concertId: string }) {
  ...
  const [allConcerts, setAllConcerts] = useState<Array<{ id: string; city: string; concertDate: string; tour: Tour }>>([])
  ...
  const load = useCallback(async () => {
    const [concertResponse, toursResponse, concertsResponse] = await Promise.all([fetch(`/api/admin/music/concerts/${concertId}`), fetch('/api/admin/music/tours'), fetch('/api/admin/music/concerts')])
    ...
    if (concertsResponse.ok) setAllConcerts(((await concertsResponse.json()).concerts || []).filter((row: { id: string }) => row.id !== concertId))
    setDirty(false)
  }, [concertId])
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-5">
    <section className="border border-sky-100 bg-white/90 p-6"><Link href="/admin/music/concerts" onClick={(event) => { if (dirty && !window.confirm('有未保存修改，确定离开吗？')) event.preventDefault() }} className="text-sm font-black text-brand-700">← 演唱会管理</Link><div className="mt-4 flex flex-wrap items-center justify-between gap-4"><div><h1 className="break-words text-4xl font-black text-brand-950">{concert.city} · {concert.concertDate.slice(0,10)}</h1><p className="mt-2 text-sm font-bold text-slate-500">{dirty ? '有未保存修改' : concert.status === 'PUBLISHED' ? '已发布' : '草稿'}</p></div><button type="button" onClick={() => void save(undefined, concert.status === 'PUBLISHED' ? 'DRAFT' : 'PUBLISHED')} className="bg-emerald-700 px-5 py-3 text-sm font-black text-white">{concert.status === 'PUBLISHED' ? '转为草稿' : '发布场次'}</button></div></section>
    ...
    <form onSubmit={save} className="space-y-6">
      ...
      <div className="sticky bottom-20 z-30 flex justify-end border border-sky-100 bg-white/95 p-3 shadow-xl md:bottom-4"><button disabled={busy} className="bg-brand-950 px-7 py-3 text-sm font-black text-white disabled:opacity-50">{busy ? '事务保存中…' : '保存基本信息、歌单和特别时刻'}</button></div>
    </form>
  </main>
}
