...
export function AdminConcertManager() {
  ...
  async function create(event: FormEvent) {
    ...
    const response = await fetch('/api/admin/music/concerts', {
      ...
      body: JSON.stringify({ tourId: form.tourId, countryOrRegion: form.countryOrRegion, city: form.city, venue: form.venue, posterUrl: form.posterUrl, status: form.status, stageType: form.stageType, startTime: form.startTime || undefined, endTime: form.endTime || undefined, concertDates, setlistSource: form.setlistSource, sourceConcertId: form.sourceConcertId, setlist }),
    })
    const data = await response.json().catch(() => null)
  ...
  return <main className="admin-mobile-page mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-5">
    ...
    {createOpen ? <form ref={createFormRef} onSubmit={create} className="scroll-mt-24 border border-sky-100 bg-white/90 p-5 shadow-sm sm:p-7">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2"><div><span className="text-sm font-black text-slate-700">演出日期（点击多选，可切换月份）</span><div className="mt-1"><MultiDatePicker value={concertDates} onChange={setConcertDates} /></div></div><label className="text-sm font-black text-slate-700">场馆<input value={form.venue} onChange={(event) => setForm((current) => ({ ...current, venue: event.target.value }))} className={`${field} mt-1`} /></label><label className="text-sm font-black text-slate-700">开始时间<input type="time" value={form.startTime} onChange={(event) => setForm((current) => ({ ...current, startTime: event.target.value }))} className={`${field} mt-1`} /></label><label className="text-sm font-black text-slate-700">结束时间<input type="time" value={form.endTime} onChange={(event) => setForm((current) => ({ ...current, endTime: event.target.value }))} className={`${field} mt-1`} /></label><label className="text-sm font-black text-slate-700">海报地址<input value={form.posterUrl} onChange={(event) => setForm((current) => ({ ...current, posterUrl: event.target.value }))} className={`${field} mt-1`} placeholder="可保存后在场次编辑页上传海报" /></label><label className="text-sm font-black text-slate-700">状态<select value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value as ConcertStatus }))} className={`${field} mt-1`}><option value="DRAFT">草稿 / 待定</option><option value="PUBLISHED">已发布</option></select></label></div>
