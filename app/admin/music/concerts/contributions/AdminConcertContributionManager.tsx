...
type Contribution = {
  ...
  createdAt: string
  submitter: { uid: number; nickname: string; displayName: string; avatarUrl: string | null }
...
type Detail = Contribution & {
  ...
  reviewer: { uid: number; nickname: string } | null
}
...
export function AdminConcertContributionManager({ initialSubmissionId, showReviewActions = false }: { initialSubmissionId?: string | null; showReviewActions?: boolean }) {
  ...
  useEffect(() => { void loadList(); if (initialSubmissionId) void loadDetail(initialSubmissionId) }, [status, type, initialSubmissionId])
  ...
  async function save() {
    ...
    else { setMessage(data?.message || '资料已保存'); await loadDetail(detail.id); await loadList() }
    setBusy(false)
  }
  ...
  return <main className="admin-mobile-page mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-5">
    <section className="border border-sky-100 bg-white/95 p-6 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div><Link href="/admin/music/concerts" className="text-sm font-black text-brand-700">← 演唱会场次管理</Link><h1 className="mt-4 text-4xl font-black text-brand-950">用户投稿资料管理</h1><p className="mt-2 text-sm font-bold text-slate-500">管理员可在资料编辑器中修正正式字段、歌曲顺序和 Encore；审核通过或拒绝请统一前往审核中心。</p></div>
        <div className="flex flex-wrap gap-2">{(['PENDING', 'APPROVED', 'REJECTED'] as const).map((value) => <button key={value} type="button" onClick={() => { setStatus(value); setDetail(null) }} className={`px-3 py-2 text-sm font-black ${status === value ? 'bg-brand-950 text-white' : 'bg-sky-50 text-brand-700'}`}>{value === 'PENDING' ? '待审核' : value === 'APPROVED' ? '已通过' : '已拒绝'}</button>)}</div>
      </div>
      <div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={() => setType('')} className={`px-3 py-2 text-xs font-black ${!type ? 'bg-sky-200 text-brand-950' : 'bg-sky-50 text-brand-700'}`}>全部类型</button>{(['SHOW', 'SETLIST', 'ENCORE'] as const).map((value) => <button type="button" key={value} onClick={() => setType(value)} className={`px-3 py-2 text-xs font-black ${type === value ? 'bg-sky-200 text-brand-950' : 'bg-sky-50 text-brand-700'}`}>{value === 'SHOW' ? '场次' : value === 'SETLIST' ? '歌单' : 'Encore'}</button>)}</div>
    </section>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(340px,0.9fr)_minmax(0,1.5fr)]">
      <section className="space-y-3">{items.map((item) => <button type="button" key={item.id} onClick={() => void loadDetail(item.id)} className={`block w-full border p-4 text-left ${detail?.id === item.id ? 'border-brand-500 bg-sky-50' : 'border-sky-100 bg-white/90'}`}><div className="flex gap-3"><div className="size-10 shrink-0 overflow-hidden bg-brand-950">{item.submitter.avatarUrl ? <img src={item.submitter.avatarUrl} alt="" className="size-full object-cover" /> : <span className="grid size-full place-items-center text-xs font-black text-white">{String(item.submitter.uid).slice(-1)}</span>}</div><div className="min-w-0 flex-1"><div className="flex flex-wrap justify-between gap-2"><strong className="break-words text-sm text-brand-950">{item.submitter.displayName} · UID {item.submitter.uid}</strong><span className="text-xs font-black text-brand-700">{item.typeLabel}</span></div><p className="mt-2 break-words text-sm font-bold text-slate-600">{showSummary(item)}</p><p className="mt-1 text-xs font-bold text-slate-400">{item.createdAt.slice(0, 16).replace('T', ' ')}</p></div></div></button>)}{!items.length ? <p className="border border-sky-100 bg-white/90 p-6 text-sm font-bold text-slate-500">当前没有投稿。</p> : null}</section>
      <section>{detail ? <section className="border border-sky-100 bg-white/95 p-5 shadow-sm sm:p-7"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black tracking-[0.18em] text-brand-600">{detail.typeLabel}</p><h2 className="mt-2 break-words text-2xl font-black text-brand-950">{detail.submitter.displayName} · UID {detail.submitter.uid}</h2><p className="mt-1 text-sm font-bold text-slate-500">展示名：{detail.submitter.displayName} · 投稿于 {detail.createdAt.slice(0, 16).replace('T', ' ')}</p></div><span className="bg-sky-50 px-3 py-1 text-xs font-black text-brand-700">{detail.statusLabel}</span></div>{duplicateWarning.length ? <div className="mt-5 border border-amber-200 bg-amber-50 p-3 text-sm font-bold text-amber-900"><p>该场次可能与现有正式场次重复，请确认后再审核通过。</p><p className="mt-2 text-xs">{duplicateWarning.map((row) => `${row.MusicTour.name} · ${row.city} · ${dateText(row.concertDate)}${row.venue ? ` · ${row.venue}` : ''}`).join('；')}</p></div> : null}{detail.type === 'SETLIST' && detail.hasFormalSetlist ? <p className="mt-4 border border-amber-200 bg-amber-50 p-3 text-sm font-black text-amber-900">该场次已经存在正式歌单，本次投稿将更新现有歌单。</p> : null}{detail.type === 'ENCORE' && detail.hasFormalEncore ? <p className="mt-4 border border-amber-200 bg-amber-50 p-3 text-sm font-black text-amber-900">该场次已经存在正式 Encore，本次投稿将更新现有 Encore。</p> : null}<label className="mt-6 block text-sm font-black text-brand-950">正式字段编辑<textarea value={payloadText} onChange={(event) => setPayloadText(event.target.value)} disabled={detail.status !== 'PENDING'} className={`${field} mt-2 min-h-[360px] font-mono text-xs`} spellCheck={false} /></label><p className="mt-2 text-xs font-bold leading-5 text-slate-500">歌单 `items` 保持正式 `MusicConcertSetlistItem` 字段；调整数组顺序即可调整歌曲顺序，`isEncore: true` 即为 Encore。服务端会再次校验曲库歌曲和场次 ID。</p>{showReviewActions ? <label className="mt-5 block text-sm font-black text-brand-950">拒绝原因<textarea value={reviewNote} onChange={(event) => setReviewNote(event.target.value)} className={`${field} mt-2 min-h-24`} placeholder="拒绝时必须填写拒绝原因" /></label> : null}<div className="mt-5 flex flex-wrap justify-end gap-2"><button type="button" onClick={save} disabled={busy || detail.status !== 'PENDING'} className="bg-sky-50 px-4 py-2 text-sm font-black text-brand-700 disabled:opacity-40">保存资料修改</button>{showReviewActions ? <><button type="button" onClick={reject} disabled={busy || detail.status !== 'PENDING'} className="bg-red-700 px-4 py-2 text-sm font-black text-white disabled:opacity-40">拒绝投稿</button><button type="button" onClick={() => void approve()} disabled={busy || detail.status !== 'PENDING' || Boolean(duplicateWarning.length)} className="bg-emerald-700 px-4 py-2 text-sm font-black text-white disabled:opacity-40">审核通过并发布</button>{duplicateWarning.length ? <button type="button" onClick={() => void approve(true)} disabled={busy || detail.status !== 'PENDING'} className="bg-amber-600 px-4 py-2 text-sm font-black text-white disabled:opacity-40">确认重复仍然发布</button> : null}</> : null}</div></section> : <div className="border border-sky-100 bg-white/90 p-8 text-sm font-bold text-slate-500">选择左侧投稿查看详情。</div>}</section>
    </div>
  </main>
}
