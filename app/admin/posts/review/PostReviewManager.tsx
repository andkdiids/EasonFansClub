...
export function PostReviewManager({ initialPosts, initialHasMore }: { initialPosts: ReviewPost[]; initialHasMore: boolean }) {
  ...
  return <section className="rounded-[28px] border border-sky-100 bg-white/90 p-5 shadow-sm sm:p-7" aria-busy={loading || Boolean(reviewingId)}>
    ...
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-sky-700">Post Queue</p><h2 className="mt-1 text-2xl font-black text-brand-950">{queueStatus === 'ALL' ? '全部帖子' : `${statusLabels[queueStatus]}帖子`}</h2></div><span className="text-sm font-black text-slate-500">{posts.length} 条</span></div>
