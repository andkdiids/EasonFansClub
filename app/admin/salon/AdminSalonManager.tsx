...
function AdminSalonRow({ post, focused, showReviewActions = false, onApprove, onReject, onEdit, onDelete }: Readonly<{ post: SalonPostView; focused?: boolean; showReviewActions?: boolean; onApprove: () => void; onReject: () => void; onEdit: () => void; onDelete: () => void }>) {
  ...
  return <article id={`salon-post-${post.id}`} data-focused={focused ? 'true' : undefined} className={`admin-salon-row${focused ? ' admin-salon-row-focused' : ''}`}><div className="admin-salon-row-images">{first ? <ImageViewer src={first.previewUrl} previewSrc={first.previewUrl} originalUrl={gallery[0]?.originalUrl} downloadUrl={gallery[0]?.downloadUrl} alt={displayTitle} gallery={gallery} imageClassName="admin-salon-image" buttonClassName="admin-salon-image-button" /> : null}<span>{post.media.length} 张 · {first?.width || 0} × {first?.height || 0}</span></div><div className="admin-salon-row-copy"><div className="admin-salon-row-meta"><b>{post.author.nickname}</b><span>UID {post.author.uid}</span><time>{new Intl.DateTimeFormat('zh-CN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(post.createdAt))}</time></div><div className="flex flex-wrap items-center gap-2"><span className={`salon-status-badge salon-status-${post.status.toLowerCase()}`}>{SALON_STATUS_LABELS[post.status]}</span><h2>{displayTitle}</h2></div><p>{context}</p>{post.content ? <div className="admin-salon-description">{post.content}</div> : null}{post.rejectReason ? <div className="salon-reject-reason">拒绝原因：{post.rejectReason}</div> : null}<div className="admin-salon-actions">{showReviewActions && post.status === 'PENDING' ? <button type="button" className="is-approve" onClick={onApprove}>通过</button> : null}{showReviewActions && (post.status === 'PENDING' || post.status === 'APPROVED') ? <button type="button" className="is-danger" onClick={onReject}>拒绝</button> : null}<button type="button" onClick={onEdit}>修改资料</button><button type="button" className="is-danger" onClick={onDelete}>删除</button></div></div></article>
}
