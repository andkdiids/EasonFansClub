...
export function AdminSalonManager({ initialPosts, initialHasMore, initialPostId, options, showReviewActions = false }: Readonly<{ initialPosts: SalonPostView[]; initialHasMore: boolean; initialPostId?: string | null; options: SalonOptions; showReviewActions?: boolean }>) {
  ...
  async function saveEdit() {
    ...
    setLoading(true); setError('')
    try {
      const allowsConcert = SALON_CATEGORY_CONFIG[editing.category].allowsConcert
      const body = {
        category: editing.category,
        tourId: allowsConcert ? editing.tourId : '',
        sessionId: allowsConcert ? editing.sessionId : '',
  ...
  return <section className="admin-salon-manager" aria-busy={loading}><div className="admin-salon-tabs" role="tablist" aria-label={showReviewActions ? '沙龙审核状态' : '沙龙作品状态'}>{SALON_POST_STATUSES.map((item) => <button key={item} type="button" role="tab" aria-selected={status === item} onClick={() => void load(item, 1)} disabled={loading}>{SALON_STATUS_LABELS[item]}</button>)}</div>{message ? <p className="salon-form-success" role="status">{message}</p> : null}{error ? <p className="salon-form-error" role="alert">{error}</p> : null}{targetNotice ? <p className="salon-form-success" role="status">{targetNotice}</p> : null}<div className="admin-salon-list">{posts.map((post) => <AdminSalonRow key={post.id} post={post} focused={post.id === initialPostId} showReviewActions={showReviewActions} onApprove={() => { setRejectReason(''); setReviewing({ post, action: 'approve' }) }} onReject={() => { setRejectReason(post.rejectReason || ''); setReviewing({ post, action: 'reject' }) }} onEdit={() => openEdit(post)} onDelete={() => void remove(post)} />)}{!posts.length ? <div className="salon-empty"><strong>暂无{SALON_STATUS_LABELS[status]}作品</strong></div> : null}</div><div className="admin-salon-pagination"><button type="button" disabled={loading || page <= 1} onClick={() => void load(status, page - 1)}>上一页</button><span>第 {page} 页</span><button type="button" disabled={loading || !hasMore} onClick={() => void load(status, page + 1)}>下一页</button></div>
...
function AdminSalonRow({ post, focused, showReviewActions = false, onApprove, onReject, onEdit, onDelete }: Readonly<{ post: SalonPostView; focused?: boolean; showReviewActions?: boolean; onApprove: () => void; onReject: () => void; onEdit: () => void; onDelete: () => void }>) {
  ...
  const context = post.concert ? formatSalonPostContext(post.category, post.concert) : '未关联演唱会'
  return <article id={`salon-post-${post.id}`} data-focused={focused ? 'true' : undefined} className={`admin-salon-row${focused ? ' admin-salon-row-focused' : ''}`}><div className="admin-salon-row-images">{first ? <ImageViewer src={first.previewUrl} previewSrc={first.previewUrl} originalUrl={gallery[0]?.originalUrl} downloadUrl={gallery[0]?.downloadUrl} alt={post.title || '沙龙作品'} gallery={gallery} imageClassName="admin-salon-image" buttonClassName="admin-salon-image-button" /> : null}<span>{post.media.length} 张 · {first?.width || 0} × {first?.height || 0}</span></div><div className="admin-salon-row-copy"><div className="admin-salon-row-meta"><b>{post.author.nickname}</b><span>UID {post.author.uid}</span><time>{new Intl.DateTimeFormat('zh-CN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(post.createdAt))}</time></div><div className="flex flex-wrap items-center gap-2"><span className={`salon-status-badge salon-status-${post.status.toLowerCase()}`}>{SALON_STATUS_LABELS[post.status]}</span><h2>{post.title || '无标题作品'}</h2></div><p>{SALON_CATEGORY_CONFIG[post.category].label} · {context}</p>{post.content ? <div className="admin-salon-description">{post.content}</div> : null}{post.rejectReason ? <div className="salon-reject-reason">拒绝原因：{post.rejectReason}</div> : null}<div className="admin-salon-actions">{showReviewActions && post.status === 'PENDING' ? <button type="button" className="is-approve" onClick={onApprove}>通过</button> : null}{showReviewActions && (post.status === 'PENDING' || post.status === 'APPROVED') ? <button type="button" className="is-danger" onClick={onReject}>拒绝</button> : null}<button type="button" onClick={onEdit}>修改资料</button><button type="button" className="is-danger" onClick={onDelete}>删除</button></div></div></article>
}
...
function EditSalonModal({ editing, options, onChange, onCancel, onSave }: Readonly<{ editing: EditTarget; options: SalonOptions; onChange: (value: EditTarget) => void; onCancel: () => void; onSave: () => void }>) {
  ...
  const categoryConfig = SALON_CATEGORY_CONFIG[editing.category]
  const allowsConcert = categoryConfig.allowsConcert
  const requiresConcert = categoryConfig.requiresConcert
  return <div className="salon-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel() }}><section className="salon-modal" role="dialog" aria-modal="true" aria-labelledby="salon-edit-title"><h2 id="salon-edit-title">修改作品资料</h2><label>分类<select value={editing.category} onChange={(event) => { const category = event.target.value as SalonCategoryValue; onChange({ ...editing, category, ...(!SALON_CATEGORY_CONFIG[category].allowsConcert ? { tourId: '', sessionId: '' } : {}) }) }}>{SALON_CATEGORIES.map((value) => <option key={value} value={value}>{SALON_CATEGORY_CONFIG[value].label}</option>)}</select></label>{allowsConcert ? <><label>演唱会<select value={editing.tourId} onChange={(event) => { const tourId = event.target.value; const tour = options.tours.find((item) => item.id === tourId); onChange({ ...editing, tourId, sessionId: tour?.sessions[0]?.id || '' }) }}><option value="">{requiresConcert ? '请选择演唱会' : '不关联演唱会'}</option>{options.tours.map((tour) => <option key={tour.id} value={tour.id}>{tour.name}</option>)}</select></label><label>场次<select value={editing.sessionId} disabled={!editing.tourId} onChange={(event) => { const sessionId = event.target.value; onChange({ ...editing, tourId: sessionId ? editing.tourId : '', sessionId }) }}><option value="">{editing.tourId ? (requiresConcert ? '请选择场次' : '不关联场次') : (requiresConcert ? '先选择演唱会' : '可选场次')}</option>{sessions.map((session) => <option key={session.id} value={session.id}>{formatSalonSession(session)}</option>)}</select></label></> : null}<label>标题<input value={editing.title} onChange={(event) => onChange({ ...editing, title: event.target.value })} /></label><label>描述<textarea value={editing.content} rows={4} onChange={(event) => onChange({ ...editing, content: event.target.value })} /></label><div><button type="button" onClick={onCancel}>取消</button><button type="button" className="is-approve" onClick={onSave}>保存修改</button></div></section></div>
}
