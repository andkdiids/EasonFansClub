...
export function AngelGiftAdminManager({ initialCampaigns }: Readonly<{ initialCampaigns: AngelGiftCampaignSummary[] }>) {
  ...
  async function openCampaign(id: string) {
    ...
    try {
      ...
      setSelectedId(id); setDetail(next); setForm(formFromCampaign(next)); setEditingPrizeId(null); setPrizeEditorMode(null); setPrizeForm(emptyPrize); setSelectedBadgeIds([]); setPrizeDrafts({}); setCollectionBackfillPreview(null); setDraws([]); setDrawPage(1); setDrawsMore(false)
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(250px,0.75fr)_minmax(0,1.6fr)]">
        ...
        <section className="min-w-0 space-y-5">
          <form className="border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900 sm:p-6" onSubmit={(event) => void saveCampaign(event)}>
            ...
            <div className="mt-5 border-t border-slate-200 pt-4 dark:border-slate-700" aria-label="全收集奖励配置"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-black text-slate-900 dark:text-slate-100">系列全收集奖励</h3><p className="mt-1 max-w-2xl text-xs font-bold leading-5 text-slate-500 dark:text-slate-400">用户集齐本主题全部已启用的普通款与隐藏款后，自动获得这枚奖励勋章。奖励勋章本身不计入收集要求，且在用户获得前不会出现在系列总数、药柜或普通用户勋章 API 中。</p></div><label className="flex shrink-0 items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300"><input type="checkbox" checked={form.collectionRewardBadgeEnabled} onChange={(event) => updateForm('collectionRewardBadgeEnabled', event.target.checked)} /> 启用全收集奖励</label></div>{form.collectionRewardBadgeEnabled ? <div className="mt-3 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]"><label><span className="admin-form-label">奖励勋章</span><select className="admin-form-input" aria-label="全收集奖励勋章" value={form.collectionRewardBadgeId} onChange={(event) => { updateForm('collectionRewardBadgeId', event.target.value); setCollectionBackfillPreview(null) }} disabled={!selectedId}><option value="">{selectedId ? '请选择奖励勋章' : '保存主题后选择奖励勋章'}</option>{detail?.collectionRewardBadge && !badges.some((badge) => badge.id === detail.collectionRewardBadge?.id) ? <option value={detail.collectionRewardBadge.id}>{detail.collectionRewardBadge.name} · {detail.collectionRewardBadge.code} · {rarityLabels[detail.collectionRewardBadge.rarity || 'COMMON'] || detail.collectionRewardBadge.rarity || '常规'}</option> : null}{badges.map((badge) => <option key={badge.id} value={badge.id}>{badge.name} · {badge.code} · {rarityLabels[badge.rarity || 'COMMON'] || badge.rarity || '常规'}</option>)}</select></label>{selectedId && form.collectionRewardBadgeId ? <div className="flex flex-wrap items-end gap-2"><button type="button" className="admin-secondary-button" disabled={busy} onClick={() => void previewCollectionBackfill()}>预览历史回填</button><button type="button" className="border border-red-700 bg-red-800 px-3 py-2 text-xs font-black text-white disabled:opacity-50" disabled={busy} onClick={() => void executeCollectionBackfill()}>确认回填</button></div> : null}</div> : null}{selectedCollectionReward && form.collectionRewardBadgeEnabled ? <div className="mt-3 flex items-center gap-3 border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/20"><BadgeIcon badge={selectedCollectionReward} size={48} /><span className="min-w-0 text-xs font-bold text-slate-700 dark:text-slate-200"><strong className="block text-sm font-black">{selectedCollectionReward.name}</strong><code className="block text-[10px] text-slate-500">{selectedCollectionReward.code || selectedCollectionReward.id}</code><span className="block text-amber-700">{rarityLabels[selectedCollectionReward.rarity || 'COMMON'] || selectedCollectionReward.rarity || '常规'}</span><span className="mt-1 block whitespace-pre-line leading-5">获取方式：{acquisitionText(selectedCollectionReward.acquisitionDescription)}</span></span></div> : null}{form.collectionRewardBadgeEnabled && !form.collectionRewardBadgeId ? <p className="mt-2 text-xs font-black text-red-600">请选择奖励勋章后再保存主题。</p> : null}{collectionBackfillPreview ? <div className="mt-3 border border-amber-300 bg-amber-50 p-3 text-xs font-bold leading-5 text-amber-900 dark:border-amber-700 dark:bg-amber-950/30 dark:text-amber-200"><strong className="block">PREVIEW · 尚未写入生产数据</strong><span>系列成员 {collectionBackfillPreview.requiredBadgeCount} 枚 · 历史满足条件 {collectionBackfillPreview.eligibleCount} 人 · 待补发 {collectionBackfillPreview.pendingCount} 人。</span></div> : null}</div>
