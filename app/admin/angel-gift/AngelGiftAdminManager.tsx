...
import { AngelGiftImageUploader } from '@/components/AngelGiftImageUploader'
...
export type AngelGiftCampaignSummary = {
  ...
  seriesBadgeCount: number
  seriesNormalCount: number
  seriesHiddenCount: number
  collectionRewardBadgeName: string | null
  createdAt: string
  updatedAt: string
}
...
type CampaignDetail = AngelGiftCampaignSummary & {
  ...
  collectionRewardBadge: PrizeBadge | null
  prizes: PrizeView[]
  stats: {
    drawCount: number
    participantCount: number
...
type CampaignForm = {
  ...
  pityIncludeHidden: boolean
  visualUrl: string
  collectionRewardBadgeEnabled: boolean
  collectionRewardBadgeId: string
  status: string
}
...
type CollectionBackfillPreview = {
  campaignId: string
...
const emptyForm: CampaignForm = { title: '', subtitle: '', description: '', startsAt: '', endsAt: '', drawCost: '', duplicateRecycleEnabled: false, duplicateRecycleRequired: '', duplicateRecycleReward: '', recycleAfterEndEnabled: true, probabilityPublic: false, dailyDrawLimit: '', totalDrawLimit: '', pityEnabled: false, pityThreshold: '10', pityIncludeHidden: false, visualUrl: '', collectionRewardBadgeEnabled: false, collectionRewardBadgeId: '', status: 'DRAFT' }
const emptyPrize: PrizeForm = { type: 'BADGE', name: '', badgeId: '', rewardAmount: '', weight: '1', quantity: '1', enabled: true, isHidden: false, sortOrder: '0' }
...
function campaignPayload(form: CampaignForm) {
  return { title: form.title, subtitle: form.subtitle || null, description: form.description || null, startsAt: toIso(form.startsAt), endsAt: toIso(form.endsAt), drawCost: Number(form.drawCost), duplicateRecycleEnabled: form.duplicateRecycleEnabled, duplicateRecycleRequired: numericOrNull(form.duplicateRecycleRequired), duplicateRecycleReward: numericOrNull(form.duplicateRecycleReward), recycleAfterEndEnabled: form.recycleAfterEndEnabled, probabilityPublic: form.probabilityPublic, dailyDrawLimit: numericOrNull(form.dailyDrawLimit), totalDrawLimit: numericOrNull(form.totalDrawLimit), pityEnabled: form.pityEnabled, pityThreshold: numericOrNull(form.pityThreshold), pityIncludeHidden: form.pityIncludeHidden, visualUrl: form.visualUrl || null, collectionRewardBadgeId: form.collectionRewardBadgeEnabled ? form.collectionRewardBadgeId || null : null, status: form.status }
}

function prizePayload(form: PrizeForm) {
  return { type: form.type, name: form.name || null, badgeId: form.type === 'BADGE' ? form.badgeId : null, rewardAmount: form.type === 'POINTS' ? Number(form.rewardAmount) : null, weight: Number(form.weight), quantity: Number(form.quantity), enabled: form.enabled, isHidden: form.type === 'BADGE' ? form.isHidden : false, sortOrder: Number(form.sortOrder) }
}
...
function prizeLabel(prize: PrizeView) {
  const label = prize.type === 'POINTS' ? `+${prize.rewardAmount ?? 0} 挂号费` : prize.badge?.name || prize.name || '未命名勋章'
  return prize.type === 'BADGE' && prize.isHidden ? `${label} · 隐藏款` : label
}
...
export function AngelGiftAdminManager({ initialCampaigns }: Readonly<{ initialCampaigns: AngelGiftCampaignSummary[] }>) {
  ...
  const [badgeSelectorPurpose, setBadgeSelectorPurpose] = useState<'MEMBER' | 'REWARD' | null>(null)
  ...
  const [collectionBackfillPreview, setCollectionBackfillPreview] = useState<CollectionBackfillPreview | null>(null)
  ...
  const selectedCollectionReward = badges.find((badge) => badge.id === form.collectionRewardBadgeId) || detail?.collectionRewardBadge || null
  ...
  async function openCampaign(id: string) {
    ...
    try {
      ...
      setSelectedId(id); setDetail(next); setForm(formFromCampaign(next)); setEditingPrizeId(null); setPrizeEditorMode(null); setBadgeSelectorPurpose(null); setPrizeForm(emptyPrize); setSelectedBadgeIds([]); setPrizeDrafts({}); setCollectionBackfillPreview(null); setDraws([]); setDrawPage(1); setDrawsMore(false); setVisualUploading(false)
  ...
  async function saveCampaign(event?: FormEvent, override?: Partial<CampaignForm>) {
    ...
    if (visualUploading) {
      setError('主题视觉图仍在上传，请稍候再保存')
      return
    }
    if (nextForm.collectionRewardBadgeEnabled && !nextForm.collectionRewardBadgeId) {
      setError('启用全收集奖励后请选择奖励勋章')
      return
    }
    if (nextForm.pityEnabled && parseAngelGiftPositiveInteger(nextForm.pityThreshold) === null) {
      setError('启用保底后请输入正整数次数')
      return
    }
  ...
  async function saveSelectedBadges(event?: FormEvent) {
    ...
    setBusy(true); setError(''); setMessage('')
    try {
      await jsonRequest(`/api/admin/angel-gift/campaigns/${selectedId}/prizes`, { method: 'POST', body: JSON.stringify({ type: 'BADGE', name: null, badgeIds: selectedBadgeIds, rewardAmount: null, weight, quantity: 1, enabled: true, isHidden: prizeForm.isHidden, sortOrder }) })
  ...
  async function previewCollectionBackfill() {
    if (!selectedId || !form.collectionRewardBadgeEnabled || !form.collectionRewardBadgeId) return
    setBusy(true); setError(''); setMessage('')
    try {
      const body = await jsonRequest(`/api/admin/angel-gift/campaigns/${selectedId}/collection-reward/preview`)
      setCollectionBackfillPreview(body.data as CollectionBackfillPreview)
      setMessage('已生成历史用户回填预览；尚未发放奖励')
    } catch (caught) { setError(caught instanceof Error ? caught.message : '生成回填预览失败') } finally { setBusy(false) }
  }

  async function executeCollectionBackfill() {
    if (!selectedId || !form.collectionRewardBadgeEnabled || !form.collectionRewardBadgeId) return
    if (!window.confirm('确认按当前系列配置，为历史上已集齐的用户补发全收集奖励吗？该操作会写入勋章获得记录。')) return
    setBusy(true); setError(''); setMessage('')
    try {
      const body = await jsonRequest(`/api/admin/angel-gift/campaigns/${selectedId}/collection-reward/execute`, { method: 'POST', body: JSON.stringify({ confirm: true }) })
      const data = body.data as { grantedCount?: number; failedCount?: number }
      setCollectionBackfillPreview(null)
      setMessage(`历史回填完成：新增 ${data.grantedCount || 0} 枚奖励${data.failedCount ? `，失败 ${data.failedCount} 位` : ''}`)
    } catch (caught) { setError(caught instanceof Error ? caught.message : '执行历史回填失败') } finally { setBusy(false) }
  }
  ...
  function startEditingPrize(prize: PrizeView) {
    const draft = prizeDrafts[prize.id] || defaultPrizeDraft(prize)
    setEditingPrizeId(prize.id); setPrizeEditorMode(prize.type === 'POINTS' ? 'POINTS' : 'BADGE'); setBadgeSelectorPurpose(null); setSelectedBadgeIds([]); setPrizeForm({ ...formFromPrize(prize), weight: draft.weight, enabled: draft.enabled })
  }

  function openBadgeSelector() {
    resetPrizeEditor(); setBadgeSelectorPurpose('MEMBER'); setPrizeEditorMode('BADGE'); setPrizeForm({ ...emptyPrize, type: 'BADGE', sortOrder: String(detail?.prizes.length || 0) }); void loadBadgeOptions()
  }
  ...
  function openCollectionRewardSelector() {
    setEditingPrizeId(null)
    setPrizeEditorMode(null)
    setSelectedBadgeIds([])
    setBadgeSelectorPurpose('REWARD')
    setBadgeSearch('')
    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(250px,0.75fr)_minmax(0,1.6fr)]">
        <section className="min-w-0 border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
          <div className="flex items-center justify-between border-b border-slate-200 p-4 dark:border-slate-700"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-amber-700 dark:text-amber-300">Themes</p><h2 className="mt-1 text-xl font-black text-slate-900 dark:text-slate-100">主题</h2></div><button type="button" className="border border-red-700 bg-red-800 px-3 py-2 text-xs font-black text-white disabled:opacity-50" disabled={visualUploading} onClick={() => { setSelectedId(null); setDetail(null); setForm(emptyForm); resetPrizeEditor(); setVisualUploading(false); setMessage(''); setError('') }}>新建主题</button></div>
          <div className="divide-y divide-slate-200 dark:divide-slate-700">{campaigns.length ? campaigns.map((campaign) => <button type="button" key={campaign.id} disabled={visualUploading} className={`block w-full p-4 text-left transition disabled:cursor-not-allowed disabled:opacity-60 ${selectedId === campaign.id ? 'bg-amber-50 dark:bg-amber-950/30' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`} onClick={() => void openCampaign(campaign.id)}><div className="flex items-start justify-between gap-3"><strong className="min-w-0 truncate text-sm font-black text-slate-900 dark:text-slate-100">{campaign.title}</strong><span className="shrink-0 text-[10px] font-black text-amber-700 dark:text-amber-300">{statusLabels[campaign.displayStatus] || campaign.displayStatus}</span></div><p className="mt-2 text-xs font-bold text-slate-500 dark:text-slate-400">执药费用 {campaign.drawCost} · 奖品 {campaign.prizeCount} 项</p><p className="mt-1 text-[10px] font-bold text-slate-500 dark:text-slate-400">系列勋章 {campaign.seriesBadgeCount} 枚（普通 {campaign.seriesNormalCount} · 隐藏 {campaign.seriesHiddenCount}）</p><p className="mt-1 text-[10px] font-bold text-amber-700 dark:text-amber-300">全收集奖励：{campaign.collectionRewardBadgeName || '未启用'}</p><p className="mt-1 text-[10px] font-bold text-slate-500 dark:text-slate-400">参与 {campaign.participantCount} 人 · 执药 {campaign.drawCount} 次 · 消耗 {campaign.drawCostTotal}</p><p className="mt-1 text-[10px] font-bold text-slate-400 dark:text-slate-500">{formatDate(campaign.startsAt)} — {formatDate(campaign.endsAt)}</p></button>) : <p className="p-6 text-center text-sm font-bold text-slate-500">还没有主题。先新建「病态三部曲」，再配置奖池。</p>}</div>
        </section>
        ...
        <section className="min-w-0 space-y-5">
          <form className="border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900 sm:p-6" onSubmit={(event) => void saveCampaign(event)}>
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="sm:col-span-2"><span className="admin-form-label">主题名称</span><input className="admin-form-input" value={form.title} onChange={(event) => updateForm('title', event.target.value)} placeholder="病态三部曲" required /></label><label><span className="admin-form-label">主题副标题</span><input className="admin-form-input" value={form.subtitle} onChange={(event) => updateForm('subtitle', event.target.value)} /></label><label><span className="admin-form-label">主题状态</span><select className="admin-form-input" value={form.status} onChange={(event) => updateForm('status', event.target.value)}>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="sm:col-span-2"><span className="admin-form-label">主题说明</span><textarea className="admin-form-input min-h-20" value={form.description} onChange={(event) => updateForm('description', event.target.value)} /></label><label><span className="admin-form-label">开始时间（北京时间）</span><input className="admin-form-input" type="datetime-local" value={form.startsAt} onChange={(event) => updateForm('startsAt', event.target.value)} /></label><label><span className="admin-form-label">结束时间（北京时间）</span><input className="admin-form-input" type="datetime-local" value={form.endsAt} onChange={(event) => updateForm('endsAt', event.target.value)} /></label><label><span className="admin-form-label">执药费用（挂号费）</span><input className="admin-form-input" type="number" min="1" step="1" value={form.drawCost} onChange={(event) => updateForm('drawCost', event.target.value)} required /></label><div className="sm:col-span-2"><AngelGiftImageUploader value={form.visualUrl || null} disabled={busy} onUploadingChange={setVisualUploading} onChange={(value) => updateForm('visualUrl', value || '')} /></div></div>
            <div className="mt-5 grid gap-3 border-t border-slate-200 pt-4 dark:border-slate-700 sm:grid-cols-2"><label className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300"><input type="checkbox" checked={form.duplicateRecycleEnabled} onChange={(event) => updateForm('duplicateRecycleEnabled', event.target.checked)} /> 启用余药回收</label><label className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300"><input type="checkbox" checked={form.recycleAfterEndEnabled} onChange={(event) => updateForm('recycleAfterEndEnabled', event.target.checked)} /> 结束后仍可回收</label><label><span className="admin-form-label">回收所需余药</span><input className="admin-form-input" type="number" min="1" step="1" value={form.duplicateRecycleRequired} onChange={(event) => updateForm('duplicateRecycleRequired', event.target.value)} disabled={!form.duplicateRecycleEnabled} /></label><label><span className="admin-form-label">回收奖励挂号费</span><input className="admin-form-input" type="number" min="1" step="1" value={form.duplicateRecycleReward} onChange={(event) => updateForm('duplicateRecycleReward', event.target.value)} disabled={!form.duplicateRecycleEnabled} /></label><label className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300"><input type="checkbox" checked={form.probabilityPublic} onChange={(event) => updateForm('probabilityPublic', event.target.checked)} /> 前台公开概率</label><label><span className="admin-form-label">每日执药上限（留空不限）</span><input className="admin-form-input" type="number" min="1" step="1" value={form.dailyDrawLimit} onChange={(event) => updateForm('dailyDrawLimit', event.target.value)} /></label><label><span className="admin-form-label">单用户主题总上限（留空不限）</span><input className="admin-form-input" type="number" min="1" step="1" value={form.totalDrawLimit} onChange={(event) => updateForm('totalDrawLimit', event.target.value)} /></label></div><div className="mt-5 border-t border-slate-200 pt-4 dark:border-slate-700" aria-label="保底机制"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-black text-slate-900 dark:text-slate-100">保底机制</h3><p className="mt-1 max-w-2xl text-xs font-bold leading-5 text-slate-500 dark:text-slate-400">达到指定执药次数后，若仍存在符合范围且用户尚未拥有的勋章，本次将按原权重抽取；中途获得新款不会重置次数。</p></div><label className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300"><input type="checkbox" checked={form.pityEnabled} onChange={(event) => updateForm('pityEnabled', event.target.checked)} /> 启用保底</label></div><div className="mt-3 grid gap-3 sm:grid-cols-2"><label><span className="admin-form-label">保底次数</span><input className="admin-form-input" type="number" min="1" step="1" value={form.pityThreshold} onChange={(event) => updateForm('pityThreshold', event.target.value)} disabled={!form.pityEnabled} /></label><fieldset className="text-xs font-bold text-slate-700 dark:text-slate-300"><legend className="admin-form-label">保底范围</legend><label className="mr-4 inline-flex items-center gap-2"><input type="radio" name="pity-range" checked={!form.pityIncludeHidden} onChange={() => updateForm('pityIncludeHidden', false)} disabled={!form.pityEnabled} /> 普通款</label><label className="inline-flex items-center gap-2"><input type="radio" name="pity-range" checked={form.pityIncludeHidden} onChange={() => updateForm('pityIncludeHidden', true)} disabled={!form.pityEnabled} /> 普通款 + 隐藏款</label></fieldset></div></div>
            <div className="mt-5 border-t border-slate-200 pt-4 dark:border-slate-700" aria-label="全收集奖励配置"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-black text-slate-900 dark:text-slate-100">系列全收集奖励</h3><p className="mt-1 max-w-2xl text-xs font-bold leading-5 text-slate-500 dark:text-slate-400">用户集齐本主题全部已启用的普通款与隐藏款后，自动获得这枚奖励勋章。奖励勋章本身不计入收集要求，且在用户获得前不会出现在系列总数、药柜或普通用户勋章 API 中。</p></div><label className="flex shrink-0 items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300"><input type="checkbox" checked={form.collectionRewardBadgeEnabled} onChange={(event) => { const enabled = event.target.checked; updateForm('collectionRewardBadgeEnabled', enabled); if (!enabled) { updateForm('collectionRewardBadgeId', ''); setCollectionBackfillPreview(null) } }} /> 启用全收集奖励</label></div>{form.collectionRewardBadgeEnabled ? <div className="mt-3 space-y-3"><div className="flex flex-wrap items-end justify-between gap-3"><div className="min-w-0 flex-1"><span className="admin-form-label">奖励勋章</span>{selectedCollectionReward ? <div className="mt-2 flex items-center gap-3 border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/20"><BadgeIcon badge={selectedCollectionReward} size={48} /><span className="min-w-0 text-xs font-bold text-slate-700 dark:text-slate-200"><strong className="block text-sm font-black">{selectedCollectionReward.name}</strong><code className="block text-[10px] text-slate-500">{selectedCollectionReward.code || selectedCollectionReward.id}</code><span className="block text-amber-700">{rarityLabels[selectedCollectionReward.rarity || 'COMMON'] || selectedCollectionReward.rarity || '常规'}</span><span className="mt-1 block whitespace-pre-line leading-5">获取方式：{acquisitionText(selectedCollectionReward.acquisitionDescription)}</span></span></div> : <p className="mt-2 text-xs font-bold text-slate-500">尚未选择奖励勋章。</p>}</div><button type="button" className="admin-secondary-button" disabled={busy} onClick={openCollectionRewardSelector}>{selectedCollectionReward ? '更换奖励勋章' : '选择奖励勋章'}</button></div>{selectedId && form.collectionRewardBadgeId ? <div className="flex flex-wrap gap-2"><button type="button" className="admin-secondary-button" disabled={busy} onClick={() => void previewCollectionBackfill()}>预览历史回填</button><button type="button" className="border border-red-700 bg-red-800 px-3 py-2 text-xs font-black text-white disabled:opacity-50" disabled={busy} onClick={() => void executeCollectionBackfill()}>确认回填</button></div> : null}</div> : null}{form.collectionRewardBadgeEnabled && !form.collectionRewardBadgeId ? <p className="mt-2 text-xs font-black text-red-600">请选择奖励勋章后再保存主题。</p> : null}{badgeSelectorPurpose === 'REWARD' ? <div className="mt-4 border border-amber-200 bg-amber-50/60 p-4 dark:border-amber-800 dark:bg-amber-950/20" aria-label="全收集奖励勋章选择器"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="text-sm font-black text-slate-900 dark:text-slate-100">选择全收集奖励勋章</h4><p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">搜索名称或 code；留空搜索会显示全部可用 Badge。已作为本主题系列成员的 Badge 不能重复设为奖励。</p></div><button type="button" className="admin-secondary-button" onClick={() => setBadgeSelectorPurpose(null)}>关闭</button></div><div className="mt-3 flex min-w-0 gap-2"><input className="admin-form-input min-w-0 flex-1" aria-label="搜索奖励 Badge 名称或 code" value={badgeSearch} onChange={(event) => setBadgeSearch(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void loadBadgeOptions(badgeSearch) } }} placeholder="搜索勋章名称或 code" /><button type="button" className="admin-secondary-button shrink-0" disabled={badgesLoading} onClick={() => void loadBadgeOptions(badgeSearch)}>{badgesLoading ? '搜索中…' : '搜索'}</button></div><div className="mt-3 max-h-[30rem] space-y-2 overflow-auto">{badges.length ? badges.map((badge) => { const joined = joinedBadgeIds.has(badge.id); const unavailable = joined || !badge.isEnabled || !badge.isActive; const selected = form.collectionRewardBadgeId === badge.id; return <button type="button" key={badge.id} disabled={unavailable} className={`flex w-full items-start gap-3 border p-3 text-left transition ${unavailable ? 'cursor-not-allowed border-slate-200 bg-slate-100 opacity-70 dark:border-slate-700 dark:bg-slate-800' : selected ? 'border-amber-500 bg-amber-100 dark:bg-amber-950/40' : 'border-slate-200 bg-white hover:border-amber-300 dark:border-slate-700 dark:bg-slate-900'}`} onClick={() => { updateForm('collectionRewardBadgeId', badge.id); setCollectionBackfillPreview(null); setBadgeSelectorPurpose(null) }}><BadgeIcon badge={badge} size={48} /><span className="min-w-0 flex-1"><strong className="block text-sm font-black text-slate-900 dark:text-slate-100">{badge.name}</strong><code className="mt-1 block text-[10px] font-bold text-slate-500 dark:text-slate-400">{badge.code}</code><span className="mt-1 block text-xs font-black text-amber-700 dark:text-amber-300">{rarityLabels[badge.rarity || 'COMMON'] || badge.rarity || '常规'}</span><span className="mt-2 block whitespace-pre-line text-xs font-bold leading-5 text-slate-600 dark:text-slate-300">当前获取方式：{acquisitionText(badge.acquisitionDescription)}</span></span><span className="shrink-0 text-[10px] font-black text-slate-500">{selected ? '当前奖励' : joined ? '已是系列成员' : !badge.isEnabled || !badge.isActive ? '不可用' : '可选'}</span></button> }) : <p className="border border-dashed border-slate-300 p-5 text-center text-xs font-bold text-slate-500 dark:border-slate-600">{badgesLoading ? '正在读取勋章…' : '没有匹配的 Badge'}</p>}</div></div> : null}{collectionBackfillPreview ? <div className="mt-3 border border-amber-300 bg-amber-50 p-3 text-xs font-bold leading-5 text-amber-900 dark:border-amber-700 dark:bg-amber-950/30 dark:text-amber-200"><strong className="block">PREVIEW · 尚未写入生产数据</strong><span>系列成员 {collectionBackfillPreview.requiredBadgeCount} 枚 · 历史满足条件 {collectionBackfillPreview.eligibleCount} 人 · 待补发 {collectionBackfillPreview.pendingCount} 人。</span></div> : null}</div>
          ...
          {selectedId && detail ? <>
            ...
            <section className="border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
              <div className="border-b border-slate-200 p-5 dark:border-slate-700">
                ...
                <div className="mt-4 border border-amber-200 bg-amber-50/60 p-3 dark:border-amber-800 dark:bg-amber-950/20" aria-label="系列勋章配置"><div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="text-sm font-black text-slate-900 dark:text-slate-100">系列勋章</h3><p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">本主题中所有 BADGE 类型奖品就是系列成员；普通款与隐藏款均可配置多枚。</p></div><span className="text-xs font-black text-amber-700 dark:text-amber-300">普通 {detail.prizes.filter((prize) => prize.type === 'BADGE' && prize.enabled && !prize.isHidden).length} · 隐藏 {detail.prizes.filter((prize) => prize.type === 'BADGE' && prize.enabled && prize.isHidden).length}</span></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" className="border border-red-700 bg-red-800 px-4 py-2.5 text-xs font-black text-white" onClick={openBadgeSelector}>＋ 添加勋章</button></div></div><div className="mt-3 flex flex-wrap gap-2" aria-label="奖池添加入口"><button type="button" className="border border-amber-600 bg-amber-500 px-4 py-2.5 text-xs font-black text-white" onClick={openPointsEditor}>＋ 添加挂号费</button></div>
              </div>

              {prizeEditorMode === 'BADGE' && badgeSelectorPurpose === 'MEMBER' && !editingPrizeId ? <form className="border-b border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50" onSubmit={(event) => void saveSelectedBadges(event)}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-black text-slate-900 dark:text-slate-100">Badge Selector</h3><p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">搜索名称或 code，可一次勾选多枚 Badge；留空搜索会显示全部可用 Badge；当前主题已加入的 Badge 会显示“已加入”。</p></div><button type="button" className="admin-secondary-button" onClick={resetPrizeEditor}>关闭</button></div><div className="mt-3 flex min-w-0 gap-2"><input className="admin-form-input min-w-0 flex-1" aria-label="搜索 Badge 名称或 code" value={badgeSearch} onChange={(event) => setBadgeSearch(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void loadBadgeOptions(badgeSearch) } }} placeholder="搜索勋章名称或 code" /><button type="button" className="admin-secondary-button shrink-0" disabled={badgesLoading} onClick={() => void loadBadgeOptions(badgeSearch)}>{badgesLoading ? '搜索中…' : '搜索'}</button></div><div className="mt-3 max-h-[30rem] space-y-2 overflow-auto">{badges.length ? badges.map((badge) => { const joined = joinedBadgeIds.has(badge.id); const selected = selectedBadgeIds.includes(badge.id); return <label key={badge.id} className={`flex items-start gap-3 border p-3 transition ${joined ? 'cursor-not-allowed border-slate-200 bg-slate-100 opacity-70 dark:border-slate-700 dark:bg-slate-800' : selected ? 'border-amber-500 bg-amber-50 dark:bg-amber-950/30' : 'border-slate-200 bg-white hover:border-amber-300 dark:border-slate-700 dark:bg-slate-900'}`}><input type="checkbox" className="mt-2 h-4 w-4" checked={selected} disabled={joined} onChange={() => toggleBadge(badge.id)} aria-label={`选择 ${badge.name}`} /><BadgeIcon badge={badge} size={48} /><span className="min-w-0 flex-1"><strong className="block text-sm font-black text-slate-900 dark:text-slate-100">{badge.name}</strong><code className="mt-1 block text-[10px] font-bold text-slate-500 dark:text-slate-400">{badge.code}</code><span className="mt-1 block text-xs font-black text-amber-700 dark:text-amber-300">{rarityLabels[badge.rarity || 'COMMON'] || badge.rarity || '常规'}</span><span className="mt-2 block whitespace-pre-line text-xs font-bold leading-5 text-slate-600 dark:text-slate-300">当前获取方式：{acquisitionText(badge.acquisitionDescription)}</span></span>{joined ? <span className="shrink-0 text-[10px] font-black text-slate-500">已加入</span> : null}</label> }) : <p className="border border-dashed border-slate-300 p-5 text-center text-xs font-bold text-slate-500 dark:border-slate-600">{badgesLoading ? '正在读取勋章…' : '没有匹配的 Badge'}</p>}</div><div className="mt-4 grid gap-3 sm:grid-cols-4"><label><span className="admin-form-label">统一初始权重</span><input className="admin-form-input" type="number" min="1" step="1" value={prizeForm.weight} onChange={(event) => updatePrize('weight', event.target.value)} /></label><label><span className="admin-form-label">起始排序</span><input className="admin-form-input" type="number" min="0" step="1" value={prizeForm.sortOrder} onChange={(event) => updatePrize('sortOrder', event.target.value)} /></label><label className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300"><input type="checkbox" checked={prizeForm.isHidden} onChange={(event) => updatePrize('isHidden', event.target.checked)} /> 设为隐藏款</label><div className="self-end"><button type="submit" className="w-full border border-red-700 bg-red-800 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50" disabled={busy || !selectedBadgeIds.length || !currentPrizeFormValid}>加入奖池（{selectedBadgeIds.length}）</button></div></div><p className="mt-2 text-xs font-bold text-slate-500 dark:text-slate-400">隐藏款在用户获得前不会出现在药柜、系列总数或普通用户勋章 API 中。</p>{!currentFormWeightValid ? <p className="mt-2 text-xs font-black text-red-600" role="alert">权重必须是正整数，当前不会产生 NaN% 或 Infinity%。</p> : null}{selectedBadges.length ? <p className="mt-3 text-xs font-bold text-slate-600 dark:text-slate-300">已选择：{selectedBadges.map((badge) => badge.name).join('、')}</p> : null}</form> : null}
