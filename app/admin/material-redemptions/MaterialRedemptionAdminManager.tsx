    ...
    <section className="space-y-5 border border-sky-100 bg-white/90 p-4 shadow-sm sm:p-6">
      ...
      {tab === 'verify' ? <div className="mx-auto max-w-2xl space-y-4">
        ...
        {verifyPreview ? <div className="border border-sky-100 bg-sky-50/50 p-4 sm:p-5">
          ...
          <dl className="mt-4 grid gap-2 border-t border-sky-100 pt-4 text-sm font-bold text-slate-600 sm:grid-cols-2"><div><dt className="text-xs text-slate-500">状态</dt><dd className="mt-1 font-black text-brand-700">{verifyPreview.expired ? '已超过核销截止时间' : verifyPreview.statusLabel}</dd></div><div><dt className="text-xs text-slate-500">兑换码</dt><dd className="mt-1 break-all font-mono font-black text-brand-950">{verifyPreview.redeemCode}</dd></div><div><dt className="text-xs text-slate-500">兑换时间</dt><dd className="mt-1">{new Date(verifyPreview.createdAt).toLocaleString('zh-CN', { hour12: false })}</dd></div><div><dt className="text-xs text-slate-500">核销截止</dt><dd className="mt-1">{new Date(verifyPreview.material.redeemEndAt).toLocaleString('zh-CN', { hour12: false })}</dd></div></dl>
          ...
          <button type="button" onClick={() => void confirmVerify()} disabled={busy || !verifyPreview.canRedeem} className="mt-4 min-h-11 w-full bg-emerald-700 px-5 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40">{verifyPreview.canRedeem ? '确认核销' : verifyPreview.expired ? '已超过核销截止时间' : verifyPreview.statusLabel}</button>
        </div> : null}
