...
export function ForgotPasswordForm({ emailEnabled, securityEnabled }: { emailEnabled: boolean; securityEnabled: boolean }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {!emailEnabled ? <p className="rounded-2xl bg-amber-50 px-4 py-3 text-sm font-black text-amber-800">邮箱重置功能暂未开放</p> : null}
      ...
      {!resetToken ? <input value={identifier} onChange={(event) => setIdentifier(event.target.value)} className="w-full rounded-xl border border-sky-100 px-4 py-3 font-bold outline-none focus:border-brand-500" placeholder="手机号、用户名或邮箱" /> : null}
      {method === 'security' && !challenge ? <button disabled={busy || !identifier} onClick={loadQuestions} className="w-full rounded-xl bg-brand-700 px-4 py-3 font-black text-white disabled:opacity-50">下一步</button> : null}
      {method === 'security' && questions.length === 3 && !resetToken ? <div className="space-y-4">{questions.map((item, index) => <label key={item.sortOrder} className="block"><span className="text-sm font-black text-brand-950">{item.question}</span><input value={answers[index]} onChange={(event) => setAnswers((current) => current.map((answer, answerIndex) => answerIndex === index ? event.target.value : answer))} className="mt-2 w-full rounded-xl border border-sky-100 px-4 py-3 font-bold" placeholder="答案" /></label>)}<button disabled={busy} onClick={verifyAnswers} className="w-full rounded-xl bg-brand-700 px-4 py-3 font-black text-white disabled:opacity-50">验证全部答案</button></div> : null}
      {method === 'email' && emailEnabled && !resetToken ? <div className="space-y-3"><button disabled={busy || !identifier} onClick={sendEmailCode} className="w-full rounded-xl bg-brand-700 px-4 py-3 font-black text-white disabled:opacity-50">发送验证码</button><input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" className="w-full rounded-xl border border-sky-100 px-4 py-3 font-bold" placeholder="6 位验证码" /><button disabled={busy || code.length !== 6} onClick={verifyEmailCode} className="w-full rounded-xl bg-brand-950 px-4 py-3 font-black text-white disabled:opacity-50">验证邮箱</button></div> : null}
      {resetToken ? <div className="space-y-3"><input type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} className="w-full rounded-xl border border-sky-100 px-4 py-3 font-bold" placeholder="新密码（至少 8 位）" /><input type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="w-full rounded-xl border border-sky-100 px-4 py-3 font-bold" placeholder="确认新密码" /><button disabled={busy || password.length < 8 || password !== confirmPassword} onClick={resetPassword} className="w-full rounded-xl bg-brand-950 px-4 py-3 font-black text-white disabled:opacity-50">重置密码</button></div> : null}
