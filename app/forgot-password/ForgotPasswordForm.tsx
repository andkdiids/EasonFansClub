...
export function ForgotPasswordForm({ emailEnabled, securityEnabled }: { emailEnabled: boolean; securityEnabled: boolean }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {!resetToken ? <input value={identifier} onChange={(event) => setIdentifier(event.target.value)} className="w-full rounded-xl border border-sky-100 px-4 py-2 font-bold outline-none focus:border-brand-500" placeholder="手机号、登录账号或邮箱" /> : null}
