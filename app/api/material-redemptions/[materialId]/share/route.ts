...
import {
  findShareableMaterial,
  materialShareMessageContent,
  toMaterialShareMessageView,
  toMaterialShareSnapshot,
} from '@/lib/material-share-service'
import { MATERIAL_SHARE_MESSAGE_TYPE, parseMaterialShareSnapshot } from '@/lib/material-share-types'
...
type StoredMaterialShareMessage = Prisma.DirectMessageGetPayload<{ select: typeof messageSelect }>
...
type ShareRecipientResult = {
  ...
  message: StoredMaterialShareMessage | null
}
...
export async function GET(request: Request, { params }: Params) {
  ...
  const limited = await enforceApiRateLimit(request, guard.user.id, {
    endpoint: '/api/material-redemptions/share/recent-friends',
  ...
  if (!material) return NextResponse.json({ message: '物料不存在或暂不可分享' }, { status: 404, headers: privateHeaders })
...
export async function POST(request: Request, { params }: Params) {
  ...
  if (!guard.user) return guard.response
  const limited = await enforceApiRateLimit(request, guard.user.id, {
    endpoint: '/api/material-redemptions/share/send',
  ...
  const material = await findShareableMaterial(materialId)
  if (!material) return shareFailure(404, 'MATERIAL_NOT_SHAREABLE', '物料不存在或暂不可分享')

  const snapshot = toMaterialShareSnapshot(material)
  const now = new Date()
  ...
  for (const { recipientId, clientMessageId } of clientMessageIds) {
    ...
    const result = await createMaterialShareMessage({
  ...
  if (recipientIds.length === 1) {
    ...
    return NextResponse.json({
      ...
      message: serializeMaterialShareMessage(result.message!, snapshot),
  ...
  return NextResponse.json({
    ...
    results: results.map((result) => ({
      ...
      duplicate: result.duplicate,
      awardedAmount: result.awardedAmount,
      ...(result.success ? { message: serializeMaterialShareMessage(result.message!, snapshot) } : { code: result.code, error: result.error }),
    })),
...
async function createMaterialShareMessage(input: {
  senderId: string
  recipientId: string
  clientMessageId: string
  snapshot: ReturnType<typeof toMaterialShareSnapshot>
  now: Date
}): Promise<ShareRecipientResult & { conversationId: string }> {
  const content = materialShareMessageContent(input.snapshot)
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        const existingSnapshot = parseMaterialShareSnapshot(existing.metadata)
        if (existing.conversationId !== conversation.id || existingSnapshot?.materialId !== input.snapshot.materialId) {
      ...
      const message = await tx.directMessage.create({
        data: {
          ...
          type: MATERIAL_SHARE_MESSAGE_TYPE,
          content,
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
      ...
      const duplicateSnapshot = duplicate ? parseMaterialShareSnapshot(duplicate.metadata) : null
      if (duplicate && duplicateSnapshot?.materialId === input.snapshot.materialId) {
    ...
    console.error('[material-share.send]', { name: error instanceof Error ? error.name : 'UnknownError' })
...
function serializeMaterialShareMessage(storedMessage: StoredMaterialShareMessage, snapshot: ReturnType<typeof toMaterialShareSnapshot>) {
  return {
    id: storedMessage.id,
    type: MATERIAL_SHARE_MESSAGE_TYPE,
    ...
    readAt: null,
    materialShare: toMaterialShareMessageView(snapshot),
  }
}
