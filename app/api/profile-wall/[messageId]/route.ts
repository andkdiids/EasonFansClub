...
export async function DELETE(_request: Request, { params }: { params: Promise<{ messageId: string }> }) {
  ...
  if (!message || message.deletedAt) return NextResponse.json({ message: 'Message not found' }, { status: 404 })

  const canDelete = viewer.id === message.senderId || viewer.role === 'ADMIN' || viewer.role === 'SUPER_ADMIN'
  if (!canDelete) return NextResponse.json({ message: 'No permission to delete this message' }, { status: 403 })
