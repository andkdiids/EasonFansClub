...
export async function POST(request: Request) {
  ...
  if (entityType !== 'album' && entityType !== 'song') return NextResponse.json({ message: '封面目标类型无效' }, { status: 400 })
  ...
  if (!exists) return NextResponse.json({ message: entityType === 'album' ? '专辑不存在' : '歌曲不存在' }, { status: 404 })
  ...
  const objectPath = `music-cover/${entityType === 'album' ? 'albums' : 'songs'}/${entityId}/cover.webp`
  ...
  else await prisma.musicSong.update({ where: { id: entityId }, data: { coverUrl: url } })
