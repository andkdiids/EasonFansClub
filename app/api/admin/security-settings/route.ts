...
export async function PUT(request: Request) {
  ...
  if (!settings) return NextResponse.json({ message: '账号安全设置格式不正确' }, { status: 400 })
  const before = await getAccountSecuritySettings()
  await prisma.$transaction(async (tx) => {
    await setAccountSecuritySettings(settings, tx)
    await tx.adminAction.create({ data: { adminId: guard.user.id, action: 'UPDATE_SETTING', reason: '更新账号安全设置', metadata: { before, after: settings } } })
  })
  return NextResponse.json({ message: '账号安全设置已保存并立即生效', settings })
}
