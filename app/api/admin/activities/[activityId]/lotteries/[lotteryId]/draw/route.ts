...
import { ActivityLotteryError, drawActivityLottery } from '@/lib/activity-lottery'
...
export async function POST(request: Request, { params }: { params: Promise<{ activityId: string; lotteryId: string }> }) {
  ...
  try {
    ...
    console.error('[admin.activities.lotteries.draw]', error instanceof Error ? error.message : error)
    return NextResponse.json({ ok: false, message: '开奖失败，请稍后重试' }, { status: 500, headers: privateHeaders })
  }
}
