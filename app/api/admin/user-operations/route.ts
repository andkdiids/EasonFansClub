...
export async function GET(request: Request) {
  ...
  // Keep the legacy themeId query working for bookmarked admin URLs, while
  // using campaignId as the authoritative Angel Gift grouping key.
  const campaignId = sanitizeText(params.get('campaignId') || params.get('themeId'), 191).trim() || null
  ...
  if (view === 'timeline') {
    ...
    const result = await getUserOperationTimeline({ userId, category, campaignId, days, page, pageSize })
  ...
  return NextResponse.json(await getTodayUserOperations({ query, category, campaignId, page, pageSize }), { headers: privateHeaders })
}
