...
export async function GET(request: Request) {
  ...
  const themeId = sanitizeText(params.get('themeId'), 191).trim() || null
  ...
  if (view === 'timeline') {
    ...
    const result = await getUserOperationTimeline({ userId, category, themeId, days, page, pageSize })
  ...
  return NextResponse.json(await getTodayUserOperations({ query, category, themeId, page, pageSize }), { headers: privateHeaders })
}
