...
async function handle(request: Request, context: RouteContext, active: boolean) {
  ...
  if (!user) return unauthenticatedResponse('请先登录后再收藏')
  const limited = await enforceApiRateLimit(request, user.id, {
    endpoint: '/api/studio/projects/favorite',
  ...
  const state = await setStudioProjectInteraction({ projectId, userId: user.id, kind: 'favorite', active })
