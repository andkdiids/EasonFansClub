...
export const securityQuestionCount = 3
...
export function validateSecurityQuestions(questions: SecurityQuestionInput[]) {
  if (questions.length !== securityQuestionCount) return '必须完整设置 3 个密保问题'
  ...
  const normalizedQuestions = questions.map((item) => item.question.toLocaleLowerCase('zh-CN'))
  if (new Set(normalizedQuestions).size !== securityQuestionCount) return '三个密保问题不能相同'
  return null
}
...
export async function ensureSecurityQuestionNotification(userId: string) {
  ...
  await prisma.notification.upsert({
    ...
    create: {
      ...
      type: 'SYSTEM',
      title: '请设置账号密保问题',
      content: '设置三个仅你知道答案的密保问题，可用于安全地找回账号。',
